...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy')
{
    ...
    //售后保障
    create_html_editor_after('goods_after_sales', htmlspecialchars($goods['goods_after_sales'])); /* 修改 by www.68ecshop.com 百度编辑器 */
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, rank_integral, suppliers_id,goods_after_sales)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$is_promote','$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$goods_img', '$goods_thumb', '$original_img', ".
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$goods_weight', '$goods_number',".
                    " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', '$is_on_sale', '$is_alone_sale', $is_shipping, ".
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$rank_integral', '$suppliers_id','$_POST[goods_after_sales]')";
        }
        else
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, extension_code, rank_integral,goods_after_sales)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$is_promote','$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$goods_img', '$goods_thumb', '$original_img', ".
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$goods_weight', '$goods_number',".
                    " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', 0, '$is_on_sale', '$is_alone_sale', $is_shipping, ".
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$code', '$rank_integral','$_POST[goods_after_sales]')";
        }
    }
    else
    {
        ...
        $sql .= "keywords = '$_POST[keywords]', " .
				...
				"supplier_status='$_POST[supplier_status]', supplier_status_txt = '$_POST[supplier_status_txt]', ". //代码增加  By www.68ecshop.com
                ...
                "goods_after_sales = '$_POST[goods_after_sales]', " .
            "last_update = '". gmtime() ."', ".
 ...
 //         $gallery_img = reformat_image_name('gallery', $goods_id, $gallery_img, 'goods');
    ...
    /* 代码修改 By www.ecshop68.com 将下面这行代码注释掉 */
...
elseif ($_POST['act'] == 'export_goods')
{
	...
	{  
		...
		while ($row=$db->fetchRow($res))
		{
			...
			{
				$Result = get_pailie_zuhe($specifications);
				foreach($Result as $key_result=> $value_result)
				{
					$result_item = "";
					$result_value = "";
					foreach($value_result as $val_key=>$val_result)
					{		
							$result_item  .= $result_item ? "\n" : "";
							$result_item  .= $val_key.":".$val_result['attr_value'];
							$result_value .= $result_value ? "|" : "";
							$result_value .=  $val_result['goods_attr_id'];			 
					}
...
elseif ($_REQUEST['act'] == 'drop_goods')
{
	...
	$ginfo = $db->getAll("select inout_rec_id from ".$ecs->table('store_inout_goods')." where goods_id=".$goods_id);
	if(count($ginfo)>0){
		$sql = "DELETE FROM " . $ecs->table('store_goods_stock') . " WHERE goods_id = '$goods_id'";
		$db->query($sql);
		$sql = "DELETE FROM " . $ecs->table('store_inout_list') . " WHERE rec_id in (".implode(',',$ginfo).")";
		$db->query($sql);
		$sql = "DELETE FROM " . $ecs->table('store_inout_note') . " WHERE inout_rec_id in (".implode(',',$ginfo).")";
		$db->query($sql);
		$sql = "DELETE FROM " . $ecs->table('store_inout_xushibu') . " WHERE inout_rec_id in (".implode(',',$ginfo).")";
		$db->query($sql);
	}
...
function get_min_price ($goods_id)
{
     $sql="SELECT g.goods_name, g.goods_type, g.promote_price, g.promote_start_date, g.promote_end_date, ".
				...
				" WHERE g.goods_id=$goods_id and a.attr_type=1";
	 $res = $GLOBALS['db']->query($sql);
	 $min_price_attr = 0;
	 while($row=$GLOBALS['db']->fetchRow($res))
	 {      
	 ...
	 return  array('min_price' =>$min_price, 'goods_name'=>$goods_name);
}
