 ...
 * @param   string  input_value 输入框值
 */
function create_html_editor_after($input_name, $input_value = '')
{
    global $smarty;
    $HTML='
    <textarea name="'.$input_name.'" id="'.$input_name.'" style="width:100%;">'.
    ...
    $smarty->assign('FCKeditor_after', $HTML);
}
/* 修改 by www.68ecshop.com 百度编辑器 end */
...
function area_list($region_id)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		$row['type'] .= ($row['region_type'] == 4) ? '五级地区' : '';
...
{
		$sql="select store_id from ". $GLOBALS['ecs']->table('store_main') ." where supplier_id=0 and parent_id = '$sid' ";
...
function get_store_fullname($store_id)
{
		...
		if($store_row['parent_id'])
		{
			...
			$store_name  = $store_name_p ?  $store_name_p."<br>".$store_name :  $store_name;
		}
		return $store_name;
}

/* 获取属性名称 */
function get_attr_name($attr_list)
{

	if(trim($attr_list))
	{
		$attr_list = str_replace("|", ",", $attr_list );
		$sql= "select attr_value from ". $GLOBALS['ecs']->table('goods_attr') ." where goods_attr_id in ($attr_list) ORDER BY  FIND_IN_SET(goods_attr_id,'$attr_list') ";
		$attr_name_arr = $GLOBALS['db']->query($sql);
		while ($row_attr_name = $GLOBALS['db']->fetchRow($attr_name_arr ))
		{
			$attr_name_list .=  $row_attr_name['attr_value']." ";
		}
...
function get_inout_type_name($typeid)
{
	if (empty($typeid))
	{
		return '';
	...
	{
		$return = $GLOBALS['db']->getOne("select type_name from ". $GLOBALS['ecs']->table('store_inout_type') ." where type_id='$typeid' ");
		return $return;
	}
}
...
/* 更新库存表（出库） */
function update_stock_out($rec_id, $store_id)
{
	$sql = "select goods_id, attr_value, number_shishou, supplier_id from ". $GLOBALS['ecs']->table('store_inout_goods') ." where inout_rec_id='$rec_id' ";
	$res = $GLOBALS['db']->query($sql);
	$no_jian=0;
	$no_jian_item="商品ID：";
	$update_arr =array();
	...
	if (!$no_jian)
	{
		foreach($update_arr AS $upd)
		{
				...
				$sql = "insert into  ".$GLOBALS['ecs']->table('store_inout_xushibu')."(inout_rec_id, goods_id, attr_value, inout_mode, number_shishou, number_stock, supplier_id) ".
						" values('$rec_id', '$upd[goods_id]',  '$upd[attr_value]', '1', '$upd[number_shishou]', '$number_stock', '$upd[supplier_id]' )";	
				$GLOBALS['db']->query($sql);
				update_old_stock($upd['goods_id'], $upd['attr_value']);
		}
	}
...
/* 更新库存表（入库） */
function update_stock_in($rec_id, $store_id)
{
	$sql = "select goods_id, attr_value, number_shishou, supplier_id from ". $GLOBALS['ecs']->table('store_inout_goods') ." where inout_rec_id='$rec_id' ";
	$res = $GLOBALS['db']->query($sql);
	while ($row= $GLOBALS['db']->fetchRow($res))
	{
		...
		{
			$sql = "insert into  ".$GLOBALS['ecs']->table('store_goods_stock')."(goods_id, store_id, goods_attr, store_number, supplier_id) ".
						" values('$row[goods_id]', '$store_id', '$row[attr_value]', '$row[number_shishou]', '$row[supplier_id]')";	
		}
		$GLOBALS['db']->query($sql);
		...
		$sql = "insert into  ".$GLOBALS['ecs']->table('store_inout_xushibu')."(inout_rec_id, goods_id, attr_value, inout_mode, number_shishou, number_stock, supplier_id) ".
						" values('$rec_id', '$row[goods_id]',  '$row[attr_value]', '2', '$row[number_shishou]', '$number_stock', '$row[supplier_id]' )";	
		$GLOBALS['db']->query($sql);
		update_old_stock($row['goods_id'], $row['attr_value']);
	}
}
