 ...
 * ============================================================================
 * $Author:  $
 * $Id: takegoods.php 17217 2011-01-19 06:29:08Z $
*/
...
if ($_REQUEST['act'] == 'set_postman')
{
	...
	$order = $db->getRow($sql);
	$order['send_region_name'] = $order['send_region_id'] ? get_regionname_ecshop68($order['send_region_id']) : "";
...
if ($_REQUEST['act'] == 'view')
{
	...
	if ($order)
	{
		...
		$order['send_region_name'] = $order['send_region_id'] ? get_regionname_ecshop68($order['send_region_id']) : "";
		$order['to_region_name'] = $order['to_region_id'] ? get_regionname_ecshop68($order['to_region_id']) : "";
    ...
    if (is_array($ids))
	{
				...
				if (isset($_POST['export']))
				{
						...
						while ($row_kd = $db->fetchRow($res_kd))
						{
							...
							$row_kd['send_region_name'] = $row_kd['send_region_id'] ? get_regionname_ecshop68($row_kd['send_region_id']) : "";
							$row_kd['send_region_name'] = $row_kd['send_name'] .' [TEL：'.$row_kd['send_tel'].']，'.$row_kd['send_region_name'].$row_kd['send_address'];
							$row_kd['to_region_name'] = $row_kd['to_region_id'] ? get_regionname_ecshop68($row_kd['to_region_id']) : "";
...
function get_kuaidi_order_list()
{
	...
	foreach($row AS $row_key =>$row_val)
	{
		...
		$row[$row_key]['send_region_name'] = $row_val['send_region_id'] ? get_regionname_ecshop68($row_val['send_region_id']) : "";
		$row[$row_key]['to_region_name'] = $row_val['to_region_id'] ? get_regionname_ecshop68($row_val['to_region_id']) : "";
...
function get_regionname_ecshop68($region_id)
{
	return $GLOBALS['db']->getOne('select region_name from '. $GLOBALS['ecs']->table('region') .' where region_id='. $region_id);
}
...
function set_order_status($o_id, $o_status)
{
		...
		{				
				$sql = "update ". $GLOBALS['ecs']->table('kuaidi_order_status')." set  status_display='1'  where status_id = '7' and order_id='$o_id' ";
				$GLOBALS['db']->query($sql);				
		}
}
?>