...
/*  代码增加_start  BY  morestock_morecity */
/*
elseif ($_REQUEST['act'] == 'inout_out')
{
		$rec_id = $_REQUEST['rec_id'] ?  intval($_REQUEST['rec_id']) : 0;
		$send_store_number = $_REQUEST['send_store_number'] ?  intval($_REQUEST['send_store_number']) : 1;
		$sql= "select o.order_id, o.order_sn, o.country, o.province, o.city, o.district, o.xiangcun, o.supplier_id, og.goods_id, og.goods_sn, og.goods_attr_id from  ".$ecs->table('order_goods')." AS og ".
					" left join ". $ecs->table('order_info') ." AS o on og.order_id=o.order_id where og.rec_id='$rec_id' ";
	    $order_row= $db->getRow($sql);
		...
		else
		{
				...
				$add_date = local_date('Ymd');
			     ...
			     $sql = "insert into ". $ecs->table('store_inout_list') ."(inout_sn, inout_status, store_id, adminer_id, inout_type, inout_mode, order_sn, ".
							"takegoods_man, today_sn, add_date, add_time, supplier_id)  ".
					       "values('$inout_sn', '3', '$store_row[store_id]', '$_SESSION[admin_id]', '2', '1', '$order_row[order_sn]', ".
					       "'$_SESSION[admin_name]', '$today_sn' , '$add_date', '$add_time', '$order_row[supplier_id]' ) ";
				 $db->query($sql);
				 $inout_rec_id = $db->insert_id();
				 $sql = "insert into ". $ecs->table('store_inout_goods') ."(inout_rec_id, goods_id, goods_sn, attr_value, inout_mode, number_yingshou, number_shishou, supplier_id)  ".
					       "values('$inout_rec_id', '$order_row[goods_id]', '$order_row[goods_sn]', '$goods_attr', '1', '$send_store_number', '$send_store_number', '$order_row[supplier_id]') ";
				 $db->query($sql);
				 $upre = update_stock_out($inout_rec_id, $store_row['store_id']);  //更新库存表
				if($upre['error'])
				{
...
elseif ($_REQUEST['act'] == 'inout_in')
{
		$rec_id = $_REQUEST['rec_id'] ?  intval($_REQUEST['rec_id']) : 0;
		$send_store_number = $_REQUEST['send_store_number'] ?  intval($_REQUEST['send_store_number']) : 1;
		$send_store_id = $_REQUEST['send_store_id'] ?  intval($_REQUEST['send_store_id']) : 0;
		$sql= "select o.order_id, o.order_sn, o.country, o.province, o.city, o.district,o.xiangcun,o.supplier_id, og.goods_id, og.goods_sn, og.goods_attr_id from  ".$ecs->table('order_goods')." AS og ".
					" left join ". $ecs->table('order_info') ." AS o on og.order_id=o.order_id where og.rec_id='$rec_id' ";
	    $order_row= $db->getRow($sql);
		$goods_attr=str_replace(",", "|", $order_row['goods_attr_id']);
				...
				$add_date = local_date('Ymd');
				 ...
				 $inout_rec_id = $db->insert_id();
				 $sql = "insert into ". $ecs->table('store_inout_goods') ."(inout_rec_id, goods_id, goods_sn, attr_value, inout_mode, number_yingshou, number_shishou, supplier_id)  ".
					       "values('$inout_rec_id', '$order_row[goods_id]', '$order_row[goods_sn]', '$goods_attr', '2', '$send_store_number', '$send_store_number', '$order_row[supplier_id]') ";
				$db->query($sql);
				$upre = update_stock_in($inout_rec_id, $send_store_id);  //更新库存表
...
{
	/* 代码增加_start  By  morestock_morecity */
	//$smarty->assign('cfg_stock_dec_time', $_CFG['stock_dec_time']);
	/* 代码增加_end  By  morestock_morecity */
...
elseif ($_REQUEST['act'] == 'delivery_ship')
{
	...
	/* 代码增加_start  By  morestock_morecity   发货操作 生成出库单并减库房库存 */
	/*
	foreach ($delivery_stock_result as $value)
        {
			echo $value['goods_id'];
	...
	if ($_CFG['use_storage'] == '1' && $_CFG['stock_dec_time'] == SDT_SHIP)
    {
        ...
        foreach ($delivery_stock_result as $value)
        {
			...
			$store_row = get_storeid_youhuo($order['province'], $order['city'], $order['district'], $order['xiangcun'], $value['goods_id'], $goods_attr_id,$order['supplier_id']); 
			...
			$add_date = local_date('Ymd');
			 foreach ($delivery_stock_result as $value)
			 {
					...
					$sql = "insert into ". $ecs->table('store_inout_list') ."(inout_sn, inout_status, store_id, adminer_id, inout_type, inout_mode, order_sn, ".
					       ...
					       "values('$inout_sn', '3', '$store_row[store_id]', '$_SESSION[admin_id]', '1', '1', '$order[order_sn]', ".
					       "'$_SESSION[admin_name]', '$today_sn' , '$add_date', '$add_time','$order[supplier_id]','$stock_type_id' ) ";
					$db->query($sql);
					$inout_rec_id = $db->insert_id();
					$sql = "insert into ". $ecs->table('store_inout_goods') ."(inout_rec_id, goods_id, goods_sn, attr_value, inout_mode, number_yingshou, number_shishou, supplier_id, store_type_id)  ".
					       "values('$inout_rec_id', '$value[goods_id]', '$value[goods_sn]', '$goods_attr_id', '1', '$value[sums]', '$value[sums]','$order[supplier_id]','$stock_type_id') ";
					$db->query($sql);
					update_stock_out($inout_rec_id, $store_row['store_id']);  //更新库存表
			 }
		}
	}
	...
	/* wei2 add by www.68ecshop.com start */
...
@file_get_contents($autoUrl.'weixin/auto_do.php?type=2');
	/* wei2 add by www.68ecshop.com  end */
            /* 取得用户信息 */
            $user = user_info($order['user_id']);
        ...
        /* 如果需要，发短信 */
        if ($GLOBALS['_CFG']['sms_order_shipped'] == '1')
        {
            include_once('../send.php');
				$content = '您的订单已发货，订单号为'.$order['order_sn'].'收货人为'.$order['consignee'].'收货地址为'.$order['address'].'，请注意查收【68ecshop】';
				sendSMS($_order['mobile'],$content);
        }
    }
	...
	if ($_CFG['use_storage'] == '1' && $_CFG['stock_dec_time'] == SDT_SHIP)
    {
        ...
        foreach ($delivery_stock_result as $key => $value)
        {
				...
				$add_date = local_date('Ymd');
			     ...
			     $sql = "insert into ". $ecs->table('store_inout_list') ."(inout_sn, inout_status, store_id, adminer_id, inout_type, inout_mode, order_sn, ".
							"takegoods_man, today_sn, add_date, add_time, supplier_id, store_type_id)  ".
					       "values('$inout_sn', '3', '$store_row[store_id]', '$_SESSION[admin_id]', '5', '2', '$order[order_sn]', ".
					       "'$_SESSION[admin_name]', '$today_sn' , '$add_date', '$add_time','$order[supplier_id]', '$stock_type_id' ) ";
				 $db->query($sql);
				 $inout_rec_id = $db->insert_id();
				 $sql = "insert into ". $ecs->table('store_inout_goods') ."(inout_rec_id, goods_id, goods_sn, attr_value, inout_mode, number_yingshou, number_shishou, supplier_id, store_type_id)  ".
					       "values('$inout_rec_id', '$value[goods_id]', '$value[goods_sn]', '$goods_attr_id', '2', '$value[send_number]', '$value[send_number]','$order[supplier_id]', '$stock_type_id') ";
				$db->query($sql);
				$upre = update_stock_in($inout_rec_id, $store_row['store_id']);  //更新库存表				
		}
		}
	}
	/* 代码增加_end   By  morestock_morecity  */
...
elseif ($_REQUEST['act'] == 'step_post')
{
    ...
    elseif ('shipping' == $step)
    {
		...
		//morestock_morecity start  只修改运送方式，不计算运费变化与订单总钱的关系
    ...
    elseif ('shipping' == $step)
    {
		...
		if(count($ret)>0){
			foreach($ret as $key => $val){
				...
				$val['configure'] = unserialize($val['configure']);
				$shipping_list[$key]['shipping_desc'] ='不足免费额度运费需支付'.$val['configure']['shipping_fee'].'元';
				$shipping_list[$key]['shipping_desc'] .='，免费额度：'.$val['configure']['free_money'].'元';
			}
		}
		//morestock_morecity end
        $smarty->assign('shipping_list', $shipping_list);
...
function order_list()
{
    ...
    if ($result === false)
    {
        ...
        //$where = 'WHERE 1 ';
        $where = ($filter['supp']>0) ? 'WHERE o.supplier_id > 0' : 'WHERE 1';//morestock_morecity
        
        if ($filter['suppid']){
        	//$where .= " AND o.supplier_id = ".$filter['suppid'];
        	$where = 'WHERE o.supplier_id = '.$filter['suppid'];//morestock_morecity
        }
		...
		if($filter['supp']==0 && $search){//如果是查看自己的订单，就获取仓库列表
			$storeid_list="";
...
function get_stock_list()
{
	...
	$supptype = (isset($_GET['supp'])) ? intval($_GET['supp']) : 0;//定义查看的订单类型(0,自营;大于0，入驻商)
	if($supptype == 0){
		/* 仓库 */
		...
		$sql = " select store_id,store_name  from ". $ecs->table('store_main')." where supplier_id=0 and ";
