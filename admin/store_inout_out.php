...
if ($_REQUEST['act'] == 'list')
{
	...
	/* 出库类型 */
	$sql = "select type_id, type_name from ". $ecs->table('store_inout_type') ." where in_out=1 and store_type_id=0 order by type_id asc";
...
if ($_REQUEST['act'] == 'add')
{
	...
	$sql="select max(today_sn) from ". $ecs->table('store_inout_list') ." where inout_mode=1 and  add_date='$inout[add_date]' and store_type_id=0 ";
...
if ($_REQUEST['act'] == 'insert')
{
	...
	foreach ($_POST['goods_id']  AS $gkey=>$gval)
	{
			...
			$sql = "insert into ". $ecs->table('store_inout_goods') ." (inout_rec_id, goods_id, goods_sn, inout_mode, attr_value,  number_yingshou, number_shishou ) ".
						"values('$inout_rec_id', '$goods_id', '$goods_sn', '1', '$attr_value', '$number_yingshou', '$number_shishou')";
			$db->query($sql);
	}    
	...
	if ($inout)
	{
		...
		while ($row= $db->fetchRow($res))
		{
			...
			if($row['supplier_id']>0){//入驻商租自营商仓库
				$row['adminer_name'] = '入驻商'.$db->getOne("select user_name from ". $ecs->table('supplier_admin_user') ." where user_id='$row[adminer_id]' ");
			}else{
...
if ($_REQUEST['act'] == 'check_insert')
{
		...
		$sql="insert into  ". $ecs->table('store_inout_note').
				...
				" values('$rec_id', '1', '$action_val', '$inout_status', '$inout_note', '$add_time')";
		$db->query($sql);
...
if ($_REQUEST['act'] == 'edit')
{
	...
	/* 出入库类型 */
	$sql = "select type_id, type_name from ". $ecs->table('store_inout_type') ." where in_out=1 order by type_id asc";
	$inout_type_list = $db->getAll($sql);
	$smarty->assign('inout_type_list', $inout_type_list);
...
if ($_REQUEST['act'] =='update')
{
	...
	foreach ($_POST['goods_id']  AS $gkey=>$gval)
	{
			...
			$number_shishou = $_POST['number_shishou'][$gkey];
			$sql = "insert into ". $ecs->table('store_inout_goods') ." (inout_rec_id, goods_id, goods_sn, inout_mode, attr_value, number_yingshou, number_shishou ) ".
						"values('$rec_id',  '$goods_id', '$goods_sn', '1', '$attr_value', '$number_yingshou', '$number_shishou')";
			$db->query($sql);
	} 
