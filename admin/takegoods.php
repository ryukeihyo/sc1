...
{  
    ...
    /* 插入数据库。 */
    $sql = "INSERT INTO ".$ecs->table('takegoods_type')." (type_name, type_money, use_start_date, use_end_date)
    VALUES ('$type_name',
            '$_POST[type_money]',
            '$use_startdate',
            '$use_enddate')";
...
if ($_REQUEST['act'] == 'gen_excel')
{
    ...
    $val = array();
    $sql = "SELECT tg.tg_id, tg.type_id, tg.tg_sn, tg.tg_pwd,  tt.type_name, tt.type_money, tt.use_start_date, tt.use_end_date ".
...
if ($_REQUEST['act'] == 'order_send')
{
	...
	$link[] = array('text' => '返回提货列表页', 'href' => 'takegoods.php?act=order_list' );
...
elseif ($_REQUEST['act'] == 'remove_order')
{
    ...
    $url = 'takegoods.php?act=query_order&' . str_replace('act=remove_order', '', $_SERVER['QUERY_STRING']);
...
function get_takegoods_list()
{
    ...
    foreach ($row AS $key => $val)
    {
		...
		$row[$key]['is_used'] =  $val['tg_order_id'] ? '<font color=#ff3300>已使用</font>' : '未使用';
    }
