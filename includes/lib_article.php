 ...
 * @return  array
 */
function get_cat_articles($cat_id, $page = 1, $size = 10 ,$requirement='',$type='')
{
    //取出所有非0的文章
    if ($cat_id == '-1')
    {
    ...
    //是否置顶
    if($type){
        $where = " AND article_type = 1";
    }

    //增加搜索条件，如果有搜索内容就进行搜索    
    if ($requirement != '')
    {
        $sql = 'SELECT article_id, title,content, author,description, add_time, file_url, open_type,keywords' .
    ...
    else 
    {
        ...
        $sql = 'SELECT article_id, title,content,description, author, add_time, file_url, open_type,keywords' .
               ' FROM ' .$GLOBALS['ecs']->table('article') .
               ' WHERE is_open = 1 AND ' . $cat_str .$where.
               ' ORDER BY article_type DESC, article_id DESC';
    }
    ...
    if ($res)
    {
        while ($row = $GLOBALS['db']->fetchRow($res))
        {
            ...
            $arr[$article_id]['content']     = cut_str($row['content'],50);
...
function cut_str($str,$len,$suffix="..."){
    if(function_exists('mb_substr')){
        if(strlen($str) > $len){
            $str= mb_substr($str,3,$len,"utf-8").$suffix;
        }
        return $str;
    }else{
        if(strlen($str) > $len){
            $str= substr($str,3,$len).$suffix;
        }
        return $str;
    }
}
...
function get_new_articles($cat_id)
{
    ...
    $sql = 'SELECT article_id, title,content,description, author, add_time, file_url, open_type' .
        ' FROM ' .$GLOBALS['ecs']->table('article') .
