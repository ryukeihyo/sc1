...
function insert_user_account($surplus, $amount)
{
    $sql = 'INSERT INTO ' .$GLOBALS['ecs']->table('user_account').
           ' (user_id, admin_user, amount, add_time, paid_time, admin_note, user_note, process_type, payment, is_paid,bank_cardnum,account_bank,account_name,alipay_account,real_name)'.
            " VALUES ('$surplus[user_id]', '', '$amount', '".gmtime()."', 0, '', '$surplus[user_note]', '$surplus[process_type]', '$surplus[payment]', 0,'$surplus[bank_cardnum]','$surplus[account_bank]','$surplus[account_name]','$surplus[alipay_account]','$surplus[real_name]')";
    $GLOBALS['db']->query($sql);
...
function get_account_log($user_id, $num, $start)
{
    ...
    if ($res)
    {
        while ($rows = $GLOBALS['db']->fetchRow($res))
        {
            ...
            //add by liujb  20160824  start 提现修改
            $rows['bank_cardnum']   = $rows['bank_cardnum'];
            $rows['account_bank']   = $rows['account_bank'];
            $rows['account_name']   = $rows['account_name'];
            $rows['alipay_account'] = $rows['alipay_account'];
            $rows['real_name']      = $rows['real_name'];
            if($rows['account_bank'] ){
                $rows['tixian_type'] = 'bank';
            }else if ($rows['alipay_account']) {
                $rows['tixian_type'] = 'alipay';
            }else{
                $rows['tixian_type'] = '';
            }
            //add by liujb  20160824  end 提现修改
            /* 会员的操作类型： 冲值，提现 */
