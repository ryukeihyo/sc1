...
{
    $sql = 'SELECT brand_id,brand_name,brand_logo,site_url FROM ' . $GLOBALS['ecs']->table('brand') . 'WHERE is_show=1 ORDER BY sort_order';
    $res = $GLOBALS['db']->getAll($sql);
...
function xuanran($array,$bz=''){
	...
	if(is_array($array)){
		foreach($array as $key => $val){
			if($val['parent_id']==0){
				$nbz = $val['id'];
				$html.='<input type="checkbox" name="channelIds" value="'.$nbz.'" class="0" id="ch'.$val['id'].'"/>'.$val['text'];
			}else{
				$nbz = $bz.'.'.$val['id'];
				$html.='<input type="checkbox" name="channelIds" value="'.$nbz.'" class="'.$bz.'" id="ch'.$nbz.'"/>'.$val['text'];
			}
			if(is_array($val['child'])){
				$html.='<ul>';
				$html.=xuanran($val['child'],$nbz);
				$html.="</ul>";
			}
		}
	}
