...
{
	$sql = "SELECT og.rec_id, og.goods_id, og.goods_name, g.goods_thumb, og.goods_sn, og.market_price, og.goods_number, " .
            "og.goods_price, og.goods_attr, og.is_real, og.parent_id, og.is_gift, " .
            "og.goods_price * og.goods_number AS subtotal, og.extension_code, package_attr_id,c.status AS comment_status  " .
            "FROM " . $GLOBALS['ecs']->table('order_goods') . "as og right join" . $GLOBALS['ecs']->table('goods') .
			"as g on og.goods_id = g.goods_id" .
            " LEFT JOIN ".$GLOBALS['ecs']->table('comment')." AS c ON og.rec_id=c.rec_id".
...
function order_fee($order, $goods, $consignee)
{
    ...
    if ($order['shipping_id'] > 0 && $total['real_goods_count'] > 0)
    {
        ...
        if (!empty($shipping_info))
        {
            ...
            file_put_contents('./weight.txt',var_export($weight_price,true));
    ...
    if (count($order['shipping_pay']) > 0 && $total['real_goods_count'] > 0){
		foreach ($goods AS $val)
	    {
			...
			$total['supplier_shipping'][$row_supp['supplier_id']]['shipping_id'] = (isset($order['shipping_pay'][$row_supp['supplier_id']])) ? intval($order['shipping_pay'][$row_supp['supplier_id']]) : 0;
		...
		// 查看购物车中是否全为免运费商品，若是则把运费赋为零
	       ...
	       $sql = 'SELECT count(*) FROM ' . $GLOBALS['ecs']->table('cart') . " AS c left join ". $GLOBALS['ecs']->table('goods') ." AS g on c.goods_id=g.goods_id WHERE g.supplier_id = '". $row_supp['supplier_id'] ."' AND $sql_where AND c.extension_code != 'package_buy' AND c.is_shipping = 0  AND rec_id in (".$_SESSION['sel_cartgoods'].")"; //jx ;
	       $shipping_count_supp = $GLOBALS['db']->getOne($sql);
		$total['shipping_fee']    = 0;
		if($shipping_count_supp){
			foreach($total['supplier_shipping'] AS $key => $supp_shipping)
			{
				//if(isset($order['shipping_pay'][$key])){
					$supp_shipping['shipping_fee'] = get_shipping_pay_money($consignee,$key,$total['goods_price_supplier'][$key],$supp_shipping['shipping_id']);//morestock_morecity
...
$sql_where = $_SESSION['user_id']>0 ? "c.user_id='". $_SESSION['user_id'] ."' " : "c.session_id = '" . SESS_ID . "' AND c.user_id=0 ";
    $sql = "SELECT c.rec_id, c.user_id, c.goods_id, c.goods_name, c.goods_sn, c.goods_number, " .
            "c.market_price, c.goods_price, c.goods_attr, c.goods_attr_id, c.is_real, c.extension_code, c.parent_id, c.is_gift, c.is_shipping, package_attr_id," .
    ...
    foreach ($arr as $key => $value)
    {
        ...
        $arr[$key]['stock_num'] = is_have_stock($value['goods_id'],$value['goods_attr_id']);//morestock_morecity
    }
...
function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0)
{
    ...
    $sql = "SELECT g.goods_name, g.goods_sn, g.is_on_sale, g.is_real, ".
                ...
                "g.goods_number, g.is_alone_sale, g.is_shipping,g.supplier_id,".
  ...
  /*  if ($GLOBALS['_CFG']['use_storage'] == 1)
    {
		//start morestock_morecity  对应仓库是否有货
		sort($spec);
		$attr_str = implode('|',$spec);
		$stockNum = is_have_stock($goods_id,$attr_str);
		if ($num > $stockNum)
        {
            $GLOBALS['err']->add(sprintf($GLOBALS['_LANG']['shortage'], $stockNum), ERR_OUT_OF_STOCK);

            return false;
        }
    	//end morestock_morecity
        //检查：商品购买数量是否大于总库存
...
/* 代码增加_start  By  www.68ecshop.com */
		...
		$row['stock_num'] = is_have_stock($row['goods_id'],$row['goods_attr_id']);//morestock_morecity
		$row['stock_dec'] = ($row['stock_num']>0) ? '有货' : '无货';//morestock_morecity
...
//删除订单及订单相关商品
function delet_data_order($orderid){
	$GLOBALS['db']->query("delete from ". $GLOBALS['ecs']->table('order_info') ." where order_id=".intval($orderid)." limit 1");
	$GLOBALS['db']->query("delete from ". $GLOBALS['ecs']->table('order_goods') ." where order_id=".intval($orderid));
}

/**
*  操作订单当中商品时，商品库存变化
* @param int $orderid   订单id
* @param int $rec_id    订单商品表中的主键
* @param int $goodsnum  修改数量
* @param string $act    操作名称(del,edit)
*/
function change_order_goods_stock($orderid,$rec_id,$goodsnum=0,$act='del'){

	if($act == 'edit' && $goodsnum==0){
		return ;
	}
	global $ecs,$db;
	$sql= "select o.order_id, o.order_sn, o.country, o.province, o.city, o.district, o.xiangcun, o.supplier_id, og.rec_id, og.goods_id, og.goods_name, og.goods_sn, og.goods_number, og.goods_attr_id, og.send_store_number, og.send_store_id from  ".$ecs->table('order_goods')." AS og ".
				" left join ". $ecs->table('order_info') ." AS o on og.order_id=o.order_id where og.rec_id='$rec_id' and og.order_id='$orderid' ";
	$ret = $db->query($sql);
	if($ret){
		$userName = $_SESSION['user_name'].'('.$_SESSION['user_id'].')';
		while($order_row= $db->fetchRow($ret)){

			$stock_type_id = 0;//默认为自营仓库
			...
			$send_store_id = $order_row['send_store_id'];
			$send_store_number = $order_row['send_store_number'];
			$goods_attr=str_replace(",", "|", $order_row['goods_attr_id']);

			$admin_id = $db->getOne("select admin_id from ". $ecs->table('store_adminer') ." where store_id=".$send_store_id." limit 1");
			...
			if($act == 'del'){
				...
				$add_date = local_date('Ymd');
				 ...
				 $sql = "insert into ". $ecs->table('store_inout_list') ."(inout_sn, inout_status, store_id, adminer_id, inout_type, inout_mode, order_sn, ".
							"takegoods_man, today_sn, add_date, add_time, supplier_id, store_type_id)  ".
						   "values('$inout_sn', '3', '$send_store_id', '$admin_id', '7', '2', '$order_row[order_sn]', ".
						   "'$_SESSION[admin_name]', '$today_sn' , '$add_date', '$add_time', '$order_row[supplier_id]', '$stock_type_id' ) ";
				 $db->query($sql);
				 $inout_rec_id = $db->insert_id();
				 $sql = "insert into ". $ecs->table('store_inout_goods') ."(inout_rec_id, goods_id, goods_sn, attr_value, inout_mode, number_yingshou, number_shishou, supplier_id, store_type_id)  ".
						   "values('$inout_rec_id', '$order_row[goods_id]', '$order_row[goods_sn]', '$goods_attr', '2', '$send_store_number', '$send_store_number', '$order_row[supplier_id]', '$stock_type_id') ";
				$db->query($sql);
				$upre = order_update_stock_in($inout_rec_id, $send_store_id);  //更新库存表

				//$sql= "update ". $ecs->table('order_goods') ." set send_store_number='', send_store_id=''  where rec_id='$rec_id' ";
				//$db->query($sql);
			}
			if($act == 'edit'){
				//修改订单中的商品，对应商品库存进出库
				$send_store_number = abs($goodsnum);
				...
				if($goodsnum <0){
					...
					$add_date = local_date('Ymd');
					 ...
					 $sql = "insert into ". $ecs->table('store_inout_list') ."(inout_sn, inout_status, store_id, adminer_id, inout_type, inout_mode, order_sn, ".
								"takegoods_man, today_sn, add_date, add_time, supplier_id, store_type_id)  ".
							   "values('$inout_sn', '3', '$send_store_id', '$admin_id', '8', '1', '$order_row[order_sn]', ".
							   "'$userName', '$today_sn' , '$add_date', '$add_time', '$order_row[supplier_id]', '$stock_type_id' ) ";
					 $db->query($sql);
					 $inout_rec_id = $db->insert_id();
					 $sql = "insert into ". $ecs->table('store_inout_goods') ."(inout_rec_id, goods_id, goods_sn, attr_value, inout_mode, number_yingshou, number_shishou, supplier_id, store_type_id)  ".
							   "values('$inout_rec_id', '$order_row[goods_id]', '$order_row[goods_sn]', '$goods_attr', '1', '$send_store_number', '$send_store_number', '$order_row[supplier_id]', '$stock_type_id') ";
					 $db->query($sql);
					 $upre = order_update_stock_out($inout_rec_id, $send_store_id);  //更新库存表
				}
			}
		}
	}
...
function change_morestorck_morecity_stock_jian($order_id){
	...
	global $ecs,$db;
	$sql= "select o.order_id, o.order_sn, o.country, o.province, o.city, o.district, o.xiangcun, o.supplier_id, og.rec_id, og.goods_id, og.goods_name, og.goods_sn, og.goods_number, og.goods_attr_id from  ".$ecs->table('order_goods')." AS og ".
				" left join ". $ecs->table('order_info') ." AS o on og.order_id=o.order_id where og.order_id='$order_id' ";
	$ret = $db->query($sql);
	if($ret){
		$userName = $_SESSION['user_name'].'('.$_SESSION['user_id'].')';
		while($order_row= $db->fetchRow($ret)){
			...
			else
			{
				...
				$admin_id = $db->getOne("select admin_id from ". $ecs->table('store_adminer') ." where store_id=".$store_row['store_id']." limit 1");
					...
					$add_date = local_date('Ymd');
					 ...
					 $sql = "insert into ". $ecs->table('store_inout_list') ."(inout_sn, inout_status, store_id, adminer_id, inout_type, inout_mode, order_sn, ".
								"takegoods_man, today_sn, add_date, add_time, supplier_id, store_type_id)  ".
							   "values('$inout_sn', '3', '$store_row[store_id]', '$admin_id', '2', '1', '$order_row[order_sn]', ".
							   "'$userName', '$today_sn' , '$add_date', '$add_time', '$order_row[supplier_id]', '$stock_type_id' ) ";
					 $db->query($sql);
					 $inout_rec_id = $db->insert_id();
					 $sql = "insert into ". $ecs->table('store_inout_goods') ."(inout_rec_id, goods_id, goods_sn, attr_value, inout_mode, number_yingshou, number_shishou, supplier_id, store_type_id)  ".
							   "values('$inout_rec_id', '$order_row[goods_id]', '$order_row[goods_sn]', '$goods_attr', '1', '$send_store_number', '$send_store_number', '$order_row[supplier_id]', '$stock_type_id') ";
					 $db->query($sql);
					 $upre = order_update_stock_out($inout_rec_id, $store_row['store_id']);  //更新库存表
					if($upre['error'])
					{
 ...
 * 改变商品库存加
 */
function change_morestorck_morecity_stock_jia($order_id){

	global $ecs,$db;
	
	$sql= "select o.order_id, o.order_sn, o.country, o.province, o.city, o.district,o.xiangcun,o.supplier_id, og.rec_id, og.goods_id, og.goods_sn, og.goods_number, og.goods_attr_id, og.send_store_id from  ".$ecs->table('order_goods')." AS og ".
				" left join ". $ecs->table('order_info') ." AS o on og.order_id=o.order_id where og.order_id='$order_id' ";
	$ret = $db->query($sql);
	if($ret){
		while($order_row= $db->fetchRow($ret)){
			...
			$add_date = local_date('Ymd');
			 ...
			 $sql = "insert into ". $ecs->table('store_inout_list') ."(inout_sn, inout_status, store_id, adminer_id, inout_type, inout_mode, order_sn, ".
						"takegoods_man, today_sn, add_date, add_time, supplier_id, store_type_id)  ".
					   "values('$inout_sn', '3', '$send_store_id', '$_SESSION[admin_id]', '6', '2', '$order_row[order_sn]', ".
					   "'$_SESSION[admin_name]', '$today_sn' , '$add_date', '$add_time', '$order_row[supplier_id]', '$stock_type_id' ) ";
			 $db->query($sql);
			 $inout_rec_id = $db->insert_id();
			 $sql = "insert into ". $ecs->table('store_inout_goods') ."(inout_rec_id, goods_id, goods_sn, attr_value, inout_mode, number_yingshou, number_shishou, supplier_id, store_type_id)  ".
...
/* 更新库存表（出库） */
function order_update_stock_out($rec_id, $store_id)
{
	$sql = "select goods_id, attr_value, number_shishou, supplier_id,store_type_id from ". $GLOBALS['ecs']->table('store_inout_goods') ." where inout_rec_id='$rec_id' ";
	$res = $GLOBALS['db']->query($sql);
	$no_jian=0;
	$no_jian_item="商品ID：";
	$update_arr =array();
	while ($row= $GLOBALS['db']->fetchRow($res))
	{
			$sql = "select stock_id, goods_id, goods_attr, store_number from ". $GLOBALS['ecs']->table('store_goods_stock') ." where supplier_id=".$row['supplier_id']." and goods_id='$row[goods_id]' and store_id='$store_id' and goods_attr='$row[attr_value]' limit 0,1";
			$row_stock = $GLOBALS['db']->getRow($sql);
			if ($row_stock['store_number'] < $row['number_shishou'])
			{
				$no_jian++;
				$no_jian_item .= $row['goods_id']."&nbsp;";
			}
			else
			{
				$update_arr[]=array(
						...
						'goods_id'=>$row_stock['goods_id'],						
						'attr_value'=>$row_stock['goods_attr'],
						'store_number'=>$row_stock['store_number'],
	...
	if (!$no_jian)
	{
		foreach($update_arr AS $upd)
		{
				$sql = "update ". $GLOBALS['ecs']->table('store_goods_stock') ." set store_number=store_number-".$upd['number_shishou']." where stock_id='$upd[stock_id]' ";
				$GLOBALS['db']->query($sql);

				//序时簿
				$number_stock= $upd['store_number']- $upd['number_shishou'];
				$sql = "insert into  ".$GLOBALS['ecs']->table('store_inout_xushibu')."(inout_rec_id, goods_id, attr_value, inout_mode, number_shishou, number_stock, supplier_id, store_type_id) ".
						" values('$rec_id', '$upd[goods_id]',  '$upd[attr_value]', '1', '$upd[number_shishou]', '$number_stock', '$upd[supplier_id]', '$upd[store_type_id]' )";	
				$GLOBALS['db']->query($sql);
				order_update_old_stock($upd['goods_id'], $upd['attr_value']);
		}
	}

	 return array('error'=>$no_jian, 'error_item'=>$no_jian_item.' 库存不足！');
}

/* 更新库存表（入库） */
function order_update_stock_in($rec_id, $store_id)
{
	$sql = "select goods_id, attr_value, number_shishou, supplier_id, store_type_id from ". $GLOBALS['ecs']->table('store_inout_goods') ." where inout_rec_id='$rec_id' ";
	$res = $GLOBALS['db']->query($sql);
	while ($row= $GLOBALS['db']->fetchRow($res))
	{
		$sql = "select stock_id, store_number from ". $GLOBALS['ecs']->table('store_goods_stock') ." where supplier_id=".$row['supplier_id']." and goods_id='$row[goods_id]' and store_id='$store_id' and goods_attr='$row[attr_value]' limit 0,1";
		$stock_row = $GLOBALS['db']->getRow($sql);
		if ($stock_row['stock_id'])
		{
			$sql = "update ".$GLOBALS['ecs']->table('store_goods_stock')." set store_number=store_number+".$row['number_shishou']." where stock_id='$stock_row[stock_id]' ";			
		}
		else
		{
			$sql = "insert into  ".$GLOBALS['ecs']->table('store_goods_stock')."(goods_id, store_id, goods_attr, store_number, supplier_id, store_type_id) ".
						" values('$row[goods_id]', '$store_id', '$row[attr_value]', '$row[number_shishou]', '$row[supplier_id]', '$row[store_type_id]')";	
		}
		$GLOBALS['db']->query($sql);

		//序时簿
		$number_stock= $stock_row['store_number']+ $row['number_shishou'];
		$sql = "insert into  ".$GLOBALS['ecs']->table('store_inout_xushibu')."(inout_rec_id, goods_id, attr_value, inout_mode, number_shishou, number_stock, supplier_id, store_type_id) ".
						" values('$rec_id', '$row[goods_id]',  '$row[attr_value]', '2', '$row[number_shishou]', '$number_stock', '$row[supplier_id]','$row[store_type_id]' )";	
		$GLOBALS['db']->query($sql);
		order_update_old_stock($row['goods_id'], $row['attr_value']);
	}
}

/* 自动更新ECSHOP原来自带的库存表 */
function order_update_old_stock($goods_id, $attr_id)
{
	$sql="select sum(store_number) from ". $GLOBALS['ecs']->table('store_goods_stock') ." where goods_id='$goods_id'";
	$all_count  = $GLOBALS['db']->getOne($sql);
	$sql="update ". $GLOBALS['ecs']->table('goods') ." set goods_number='$all_count'  where goods_id='$goods_id'";
	$GLOBALS['db']->query($sql);

    if($attr_id)
	{
		$sql="select sum(store_number) from ". $GLOBALS['ecs']->table('store_goods_stock') ." where goods_attr = '$attr_id' and goods_id='$goods_id'";
		$attr_count  = $GLOBALS['db']->getOne($sql);
		$sql="select product_id from ". $GLOBALS['ecs']->table('products') ." where goods_attr = '$attr_id' and goods_id='$goods_id' ";
		$product_id = $GLOBALS['db']->getOne($sql);
		if($product_id)
		{
			$sql="update ". $GLOBALS['ecs']->table('products') ." set product_number='$attr_count'  where product_id='$product_id' ";
		}
		else
		{
			$sql="insert into ". $GLOBALS['ecs']->table('products') ."(goods_id, goods_attr, product_number) values('$goods_id', '$attr_id', '$attr_count' ) ";
		}
		$GLOBALS['db']->query($sql);
	}
}
...
function cart_weight_price2($type = CART_GENERAL_GOODS, $supplier_id)
{
    ...
    $sql_where = $_SESSION['user_id']>0 ? "c.user_id='". $_SESSION['user_id'] ."' " : "c.session_id = '" . SESS_ID . "' AND c.user_id=0 ";//morestock_morecity

    /* 计算超值礼包内商品的相关配送参数 */
    $sql = 'SELECT goods_id, goods_number, goods_price FROM ' . $GLOBALS['ecs']->table('cart') . " as c WHERE extension_code = 'package_buy' AND ".$sql_where;//morestock_morecity
    $row = $GLOBALS['db']->getAll($sql);
    ...
    $sql    = 'SELECT SUM(g.goods_weight * c.goods_number) AS weight, ' .
                ...
                'FROM ' . $GLOBALS['ecs']->table('cart') . ' AS c '.
                'LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' AS g ON g.goods_id = c.goods_id '.
                "WHERE g.supplier_id='". $supplier_id ."' AND " .$sql_where." ".
                "AND rec_type = '$type' AND g.is_shipping = 0 AND c.extension_code != 'package_buy'";
    $row = $GLOBALS['db']->getRow($sql);
 ...
 */
 /*
function add_store_inout_goods($orderid){
	
	//查询订单下的商品加入出库表
	$sql = "select * from ".$GLOBALS['ecs']->table('order_info')." where order_id=".$orderid;
	$row = $GLOBALS['db']->getRow($sql);
	if($row){//获取商品
		$stock_type_id = 0;//默认仓库类型id
		if($row['supplier_id']>0){//如果不是自营
			$stock_type_id = $GLOBALS['db']->getOne("select type from ".$GLOBALS['ecs']->table('store_type')." where supplier_id=".$row['supplier_id']." and status=1");
		}

		$sql_goods = "select * from ".$GLOBALS['ecs']->table('order_goods')." where order_id=".$orderid;
		$ret = $GLOBALS['db']->query($sql_goods);
		if($ret){
			while($rowg = $GLOBALS['db']->fetchRow($ret)){
				$attr_value = $GLOBALS['db']->getOne("select goods_attr from ".$GLOBALS['ecs']->table('products')." where product_id=".$rowg['product_id']);//商品属性
				//生成出库号
				$inout_sn = '';
				$sql = "INSERT INTO ".$ecs->table('store_inout_list')."(inout_sn, store_id, adminer_id,  inout_type, inout_mode, order_sn,  takegoods_man, ".
						...
						"VALUES ('$inout_sn', '$_POST[sub_id]', '$_SESSION[admin_id]', '$_POST[inout_type]', '1', '$_POST[order_sn]', '$_POST[takegoods_man]', ".
							" '1', '$today_sn', '$add_date', '$add_time')";
				$db->query($sql);
				
				$sql = "insert into ". $GLOBALS['ecs']->table('store_inout_goods') ." (inout_rec_id, goods_id, goods_sn, inout_mode, attr_value,  number_yingshou, number_shishou ) ".
						"values('0', '$rowg[goods_id]', '$rowg[goods_sn]', '1', '$attr_value', '$rowg[goods_number]', '$rowg[goods_number]')";
				$GLOBALS['db']->query($sql);
			}
		}
	}
...
/*
 * 获取相应的运送方式所需要的钱
 * @param array $consignee  收货人地址
 * @param int  $suppid 店铺id
 * @param float $allmoney  各个店铺下商品总钱
 * @param int $feeid   表ecs_store_shipping_fee中的主键 
 * @retrun float 运费钱数
 */
function get_shipping_pay_money($consignee,$suppid,$allmoney,$feeid){
	...
	global $db,$ecs;
     ...
     $region['province'] = intval($consignee['province']);
     $region['city']     = intval($consignee['city']);
     $region['district'] = intval($consignee['district']);
     $region['xiangcun']  = intval($consignee['xiangcun']);
     $stock_type =0;//自营
     if($suppid>0){
     	$stock_type = $db->getOne("select type from ".$ecs->table('store_type')." where supplier_id=".$suppid);
     }
     $shipping_config = get_address_shipping_fee($region,$feeid,$stock_type);
     if($shipping_config != false){
     	$config = unserialize($shipping_config['configure']);
     	
     	if($config['shipping_fee'] <= 0){
     		return 0;
     	}else{
     		if($allmoney <= $config['free_money']){
