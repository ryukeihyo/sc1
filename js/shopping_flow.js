 ...
 * 改变余额
 */
function changeSurplus(val,suppid)
{
	/* 代码增加_start   By    www.68ecshop.com */
  ...
  Ajax.call('flow.php?step=change_surplus', 'surplus=' + val + '&suppid=' + suppid, changeSurplusResponse, 'GET', 'JSON');
}
...
function changeSurplusResponse(obj)
{
  if (obj.error)
  {
    try
    {
      document.getElementById("ECS_SURPLUS_NOTICE_"+obj.suppid).innerHTML = obj.error;
      document.getElementById('ECS_SURPLUS_'+obj.suppid).value = '0';
      document.getElementById('ECS_SURPLUS_'+obj.suppid).focus();
    }
    catch (ex) { }
  }
  else
  {
    try
    {
      document.getElementById("ECS_SURPLUS_NOTICE_"+obj.suppid).innerHTML = '';
    }
    catch (ex) { }
...
function checkConsignee(frm)
{
  ...
  //morestock_morecity start
  if (frm.elements['xiangcun'] && frm.elements['xiangcun'].length > 1)
  {
    if (frm.elements['xiangcun'].value == 0)
    {
      err = true;
      msg.push(district_not_null);
    }
...
function selectShipping(id){
    ...
    Ajax.call('search_shouhuo.php','id=' + id , re_address, 'GET', 'JSON');

}
function re_address(result){

    if(result.address_id){
        document.getElementById('consignee').value=result.consignee;
