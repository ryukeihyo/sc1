...
function submitSurplus()
{
    ...
    var pay_type         = document.getElementById("sel_name").value;
    if(pay_type=='yinhangka') {
        var amount = document.getElementById("amount").value;
        var bank_cardnum = frm.elements['bank_cardnum'].value;
        var account_name = frm.elements['account_name'].value;
        var account_bank = frm.elements['account_bank'].value;
    }else if(pay_type=='zhifubao'){
        var  amount = document.getElementById("amount1").value;
        var  alipay_account  = frm.elements['alipay_account'].value;
  ...
  else
  {
    ...
    if(pay_type=='yinhangka') {
        if (bank_cardnum.length == 0) { //银行账号
            msg += "请输入银行账号！" + "\n";
        }

        if (account_bank.length == 0) { //开户银行
            msg += "请输入开户银行！" + "\n";
        }

        if (account_name.length == 0) { //账户名称
            msg += "请输入账户名称！" + "\n";
        }
    }else {
        if (alipay_account.length == 0) { //支付宝账户
            msg += "请输入支付宝账户！" + "\n";
        }

        if (real_name.length == 0) { //真实姓名
            msg += "请输入真实姓名！" + "\n";
        }
    }
