...
if ($_REQUEST['act']=='EditAddress')
{
	...
	if ($address_id)
	{
        ...
        $xiangcun_list = get_regions(4, $address_info['district']);//morestock_morecity        
...
elseif ($_REQUEST['act']=='saveAddress')
{
	...
	$consignee = array(
			...
			'xiangcun'      => empty($address_ecshop68->xiangcun)   ? '' :   intval($address_ecshop68->xiangcun),//morestock_morecity
	...
	$order = flow_order_info();
	$flow_type = isset($_SESSION['flow_type']) ? intval($_SESSION['flow_type']) : CART_GENERAL_GOODS;
    $region            = array($consignee['country'], $consignee['province'], $consignee['city'], $consignee['district'], $consignee['xiangcun']);
    $shipping_list     = available_shipping_list($region);
	$cart_weight_price = cart_weight_price($flow_type);
    $insure_disabled   = true;
    $cod_disabled      = true;
...
/* 代码增加_end  By www.68ecshop.com */
    ...
    check_user_address($consignee);//将收货人信息设置为cookie地址
    ...
    { 			
			...
			foreach ($consignee_list_ecshop68  as $cons_key => $cons_val)
			{
				...
				$consignee_list_ecshop68[$cons_key]['address_short_name'] .=  get_region_info($cons_val['city'])."-";
				$consignee_list_ecshop68[$cons_key]['address_short_name'] .=  get_region_info($cons_val['district'])."-";
				$consignee_list_ecshop68[$cons_key]['address_short_name'] .=  get_region_info($cons_val['xiangcun'])."&nbsp;";//morestock_morecity
				$consignee_list_ecshop68[$cons_key]['address_short_name'] .=  sub_str($cons_val['address'],16);
				$consignee_list_ecshop68[$cons_key]['address_short_name'] .=  $cons_val['zipcode'] ? (",".$cons_val['zipcode']) : "";
   ...
   $smarty->assign('shop_province_list', get_regions(1, $_CFG['shop_country']));
	...
	//file_put_contents('./total.txt',var_export($total,true));
...
elseif ($_REQUEST['step'] == 'select_shipping')
{
    ...
    else
    {
       ...
       // $order['shipping_id'] = intval($_REQUEST['shipping']);
        ...
        //$shipping_info['configure'] = unserialize_config($shipping_info['configure']);
        //file_put_contents('./pay.txt',var_export($shipping_info,true));
...
elseif ($_REQUEST['step'] == 'done')
{
    ...
    if(!check_user_address($consignee)){
    	 show_message('对不起，你选择的收货地址不在配送区域内，请返回重新选择', '', '', 'warning');
    }
    ...
    if(count($cart_goods)>0){
    	foreach($cart_goods as $key => $val){
    		//morestock_morecity start
    		if($val['stock_num']<=0 || $val['stock_num']<$val['goods_number']){
    ...
    foreach ($cart_goods_new as $ckey=>$cval){
	    ...
	    if(!isset($_POST['pay_ship'][$ckey])){
	    	...
	    	$shipid = $db->getOne("select shipping_id from ".$ecs->table('store_shipping_fee')." where fee_id=".$_POST['pay_ship'][$ckey]);
	    	if($shipid){
	   ...
	   /*morestock_morecity 把配送方式的价钱算在里面*/
	    ...
	    /* 订单中的总额 */
	    $total = order_fee($order, $cart_goods, $consignee);
		unset($order['shipping_pay'][$ckey]);//morestock_morecity 去掉这条信息以免影响下订单操作
    ...
    foreach($order_info as $ok=>$order){
	    ...
	    if ($_CFG['use_storage'] == '1' && $_CFG['stock_dec_time'] == SDT_PLACE)
	    {
			...
			if($ret !== true){
				delet_data_order($order['order_id']);//将此订单数据删除掉
				show_message($ret);
			}
	    }
...
*morestock_morecity
*/
function address_user_check($userAddress){
	$address = array();
	foreach($_REQUEST['datainfo'] as $k=>$v){
		if(isset($userAddress[$v])){
			$address[$v] = $userAddress[$v];
		}else{
			$address[$v] = 0;
		}
	}
...
morestock_morecity
*/
function check_user_address($userAddress){
	$useraddress = address_user_check($userAddress);
	$where = '';
	foreach($useraddress as $k=>$v){
		$where.=" and ".$k."=".$v;
	}
	$sql = "select count(rec_id) from ".$GLOBALS['ecs']->table('store_shipping_region')." where 1".$where;
	$result = $GLOBALS['db']->getOne($sql);
	if($result>0){
		//将用户的收货地址放到cookie中
		foreach($_REQUEST['datainfo'] as $k=>$v){
			setcookie("region_".$k,$useraddress[$v]);
			$_COOKIE["region_".$k] = $useraddress[$v];
		}
		return true;
	}else{
		return false;
