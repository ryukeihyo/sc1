...
function get_products_info($goods_id, $spec_goods_attr_id)
{
    ...
    if(isset($goods_attr_array['sort']))
    {
        ...
        //获取库存morestock_morecity
        $return_array['product_number'] = is_have_stock($goods_id,$goods_attr);
    }
    return $return_array;
}
...
function get_pc_url(){
   ...
   $sql = "select value from ".$ecs->table('shop_config')." where code = 'pc_url' ";
...
/*
*  对应仓库是否有货
* $goods_attr 为 '-1' 时不对比 goods_attr 字段，用于列表页的 有货 无货
* 返回数组 array('store_id'=>'1', 'store_number'=>'3232')
*/

function get_storeid_youhuo($province, $city, $district, $xiangcun, $goods_id, $goods_attr, $supplier_id)
{
	$store_id = get_storeid_by_ssq($province, $city, $district, $xiangcun,$supplier_id);
	if (!$store_id){ return array();}
	if (!empty($goods_attr) && $goods_attr !='-1')
	{
		$sql_attr = " and goods_attr='".$goods_attr."' ";
	}
	$sql = "select s.store_id, s.store_number from ". $GLOBALS['ecs']->table('store_goods_stock') ."  AS s left join ".
				$GLOBALS['ecs']->table('store_main'). " AS m on s.store_id=m.store_id ".
		       " where m.parent_id='$store_id' and s.goods_id='$goods_id' $sql_attr order by s.store_number desc limit 1 ";
   $sub_row = $GLOBALS['db']->getRow($sql);
   if($sub_row)
   {
	   return $sub_row;
   }
	...
	{
		return array('store_id'=>$sql);
   }
}


/* 通过省市区 获取派送仓库   （注意返回的是大仓库ID，不是库房）
** $is_country 是否使用全国库
*/

function get_storeid_by_ssq($province, $city, $district, $xiangcun, $supplier_id, $is_country=0)
{
	$suppid = $GLOBALS['db']->getOne("select type from ".$GLOBALS['ecs']->table('store_type')." where supplier_id=".$supplier_id);
	$store_id =0;
	$sql = "select store_id from ".$GLOBALS['ecs']->table('store_shipping_region')." where province='$province' and city='$city' and district='$district' and xiangcun='$xiangcun' and supplier_id='$suppid' limit 1 ";
	$store_id=$GLOBALS['db']->getOne($sql);
	if (!$store_id)
	{
		$sql = "select store_id from ".$GLOBALS['ecs']->table('store_shipping_region')." where province='$province' and city='$city' and district='$district'  and supplier_id='$suppid' limit 1 ";
		$store_id=$GLOBALS['db']->getOne($sql);
		if (!$store_id)
		{
			$sql = "select store_id from ".$GLOBALS['ecs']->table('store_shipping_region')." where province='$province'  and city='$city' and supplier_id='$suppid'  limit 1 ";
			$store_id=$GLOBALS['db']->getOne($sql);
			if(!$store_id){
				$sql = "select store_id from ".$GLOBALS['ecs']->table('store_shipping_region')." where province='$province' and supplier_id='$suppid'  limit 1 ";
				$store_id=$GLOBALS['db']->getOne($sql);
			}
		}
	}
...
//获得默认城市下的district_id
/*
function get_mydistrict_id()
{
	$sql = "select region_id from ". $GLOBALS['ecs']->table('region') ."where parent_id='$_SESSION[mycity_id]' order by region_id";
