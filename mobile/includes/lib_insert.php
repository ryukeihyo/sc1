...
function insert_select_region($arr){
	...
	foreach($newpcca as $k=>$v){
		...
		{
			   $regions[$k][$row['region_id']] = $row['region_name'] ;
		}
	}
	$names = array();
	...
	$smarty->assign('levels',$pcca);//共多少级层
	$smarty->assign('divlevels',$newpcca);//每级被选中的值
...
function insert_get_shop_shipping($arr){
	...
	if($ret && $isZhiChi){
		...
		while($row = $db->fetchRow($ret)){
			...
			$selected = '';
			if(!isset($order['shipping_pay'][$suppid]) && $i==0){
				$selected = 'selected';
				$order['shipping_pay'][$suppid] = $row['fee_id'];//记录第一个被选中的配送方式的id
			}
			if(isset($order['shipping_pay'][$suppid]) && $order['shipping_pay'][$suppid] == $row['fee_id']){
				$selected = 'selected';
				$order['shipping_pay'][$suppid] = $row['fee_id'];
			}
