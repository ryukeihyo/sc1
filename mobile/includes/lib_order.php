...
function available_payment_list($support_cod, $cod_fee = 0, $is_online = false)
{
	...
	/* 代码增加_start  By  68ecshop_zhifubao */
	$ua = strtolower($_SERVER['HTTP_USER_AGENT']);
	$uachar = "/(nokia|sony|ericsson|mot|samsung|sgh|lg|philips|panasonic|alcatel|lenovo|cldc|midp|mobile)/i";
	if(($ua == '' || preg_match($uachar, $ua))&& !strpos(strtolower($_SERVER['REQUEST_URI']),'wap'))
	{
		$sql .= "and (pay_code='alipay' or pay_code='cod' or pay_code='weixin') ";
	}
    /* 代码增加_end  By  68ecshop_zhifubao */
...
function order_fee($order, $goods, $consignee)
{
    ...
    if (count($order['shipping_pay']) > 0 && $total['real_goods_count'] > 0){
		...
		// 查看购物车中是否全为免运费商品，若是则把运费赋为零
	       ...
	       $sql = 'SELECT count(*) FROM ' . $GLOBALS['ecs']->table('cart') . " AS c left join ". $GLOBALS['ecs']->table('goods') ." AS g on c.goods_id=g.goods_id WHERE g.supplier_id = '". $row_supp['supplier_id'] ."' AND $sql_where AND c.extension_code != 'package_buy' AND c.is_shipping = 0";
