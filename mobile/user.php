...
$not_login_arr =
array('login','act_login','register','act_register','act_edit_password','get_password','send_pwd_email','password', 'signin', 'add_tag', 'collect', 'return_to_cart', 'logout', 'email_list', 'validate_email', 'send_hash_mail', 'order_query', 'is_registered', 'check_email','clear_history','qpassword_name', 'get_passwd_question', 'check_answer', 'check_register', 'oath' , 'oath_login', 'other_login');

/* 显示页面的action列表 */
$ui_arr = array('register', 'login', 'profile', 'order_list', 'order_detail', 'address_list', 'collection_list',
'message_list', 'tag_list', 'get_password', 'reset_password', 'booking_list', 'add_booking', 'account_raply',
'account_deposit', 'account_log', 'account_detail', 'act_account', 'pay', 'default', 'bonus', 'group_buy', 'group_buy_detail', 'affiliate', 'comment_list','validate_email','track_packages', 'transform_points','qpassword_name', 'get_passwd_question', 'check_answer','check_register');
...
if ($action == 'act_supplier_reg')
{
	...
	if($supplier_id)
	{
		...
		$sql="update ".$ecs->table('supplier')." set supplier_name='$supplier_name', rank_id='$rank_id', company_name='$company_name', ".
					"country='$country', province='$province', city='$city', district='$district', address='$address', tel='$tel', guimo='$guimo', email='$email', ".
					"company_type='$company_type', bank='$bank', ". $sql_img ." contact='$contact', contact_back='$contact_back', contact_shop='$contact_shop', contact_yunying='$contact_yunying', contact_shouhou='$contact_shouhou', contact_caiwu='$contact_caiwu', contact_jishu='$contact_jishu',".
					"status='0' ".
	...
	else
	{
		...
		$sql="insert into ". $ecs->table('supplier') ."(user_id, supplier_name, rank_id, company_name, country, province, city, district, address, tel, guimo, email,".
				"company_type, bank, zhizhao, id_card, contact, contact_back, contact_shop, contact_yunying, contact_shouhou, contact_caiwu, contact_jishu, add_time) ".
				" values('$user_id', '$supplier_name', '$rank_id', '$company_name', '$country', '$province', '$city', '$district', '$address', '$tel', '$guimo', '$email', ".
				"'$company_type', '$bank',  '$zhizhao_img', '$id_card_img',  '$contact', '$contact_back', '$contact_shop', '$contact_yunying', '$contact_shouhou', '$contact_caiwu', '$contact_jishu', '$add_time')";
	}
	$db->query($sql);
	show_message($mes, '返回上一页', 'user.php?act=supplier_reg', 'info');
}
if ($action == 'act_supplier_del')
{
...
elseif($action == 'check_mobile')
{
	if ($_GET['mobile'])
	{
		$sql="select user_id from ".$ecs->table('users')." where mobile_phone = '$_GET[mobile]' ";
...
elseif ($action == 'act_register')
{
    ...
    else
    {
	    ...
	    {
			$email = $username;
		    $username = mc_random(8,$zimu);
			
	    }	
	    else if(ismobile($username))
		{
			$sql = "SELECT COUNT(user_id) FROM " . $ecs->table('users') . " WHERE mobile_phone = '$username'";
			...
			else
			{
				 if ($_CFG['sms_register'] == '1')
				{
					...
					{
						/* 验证手机号验证码和IP */
						$sql = "SELECT COUNT(id) FROM " . $ecs->table('verifycode') ." WHERE mobile='$username' AND verifycode='$verifycode' AND getip='" . real_ip() . "' AND status=1 AND dateline>'" . gmtime() ."'-86400";//验证码一天内有效
						...
						else
						{
							 ...
							 $email = mc_random(6,$shuzi).mc_random(6,$zimu)."@68ecshop.com";
							 $username = mc_random(8,$zimu);							 
						}
					}
				}
...
//dqy add start 2014-3-14
elseif($action == 'check_register')
{
...
elseif ($action == 'login')
{
	...
	$smarty->assign('sms_register',$_CFG['sms_register']);
	  ...
	  /* 代码增加 start by www.68ecshop.com  */	
		$shuzi = "0123456789";
		$zimu = "abcdefghijklmnopqrstuvwxyz";
...
elseif ($action == 'act_edit_address')
{
    ...
    $address = array(
		...
		'xiangcun'   => isset($_POST['xiangcun'])  ? intval($_POST['xiangcun']) : 0,//morestock_morecity
        'address'    => isset($_POST['address'])   ? trim($_POST['address'])    : '',
