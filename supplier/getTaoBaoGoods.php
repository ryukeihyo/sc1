...
if(!$id){
	...
	$json = file_get_contents("http://hws.alicdn.com/cache/mtop.wdetail.getItemDescx/4.1/?data=%7B%22item_num_id%22%3A%22{$id}%22%7D");
	...
	if($content){
		//if($istitle or $iscomment){
			$json = file_get_contents("http://hws.alicdn.com/cache/wdetail/5.0/?id={$id}");
			$data = json_decode($json,1);
...
function getEvalution($userNumId,$id,$cnum=20){
	...
	if($allpage >= 1){
		...
		for($ipage=1;$ipage<=$allpage;$ipage++){
			$str = file_get_contents("http://rate.taobao.com/feedRateList.htm?userNumId={$userNumId}&auctionNumId={$id}&siteID=1&currentPageNum={$ipage}&order=1&content=1");
