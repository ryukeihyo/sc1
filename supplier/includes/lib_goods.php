...
function goods_list($is_delete, $real_goods=1, $conditions = '')
{
    ...
    if ($result === false)
    {
        ...
        $filter['suppliers_id'] = isset($_REQUEST['suppliers_id']) ? (empty($_REQUEST['suppliers_id']) ? $_SESSION['supplier_id'] : trim($_REQUEST['suppliers_id'])) : '';
        ...
        /* 记录总数 */
        $sql = "SELECT COUNT(*) FROM " .$GLOBALS['ecs']->table('goods'). " AS g,".$GLOBALS['ecs']->table('supplier_goods_cat')." as sgc WHERE sgc.goods_id=g.goods_id AND is_delete='$is_delete' AND sgc.supplier_id='". $_SESSION['supplier_id'] ."' $where";
