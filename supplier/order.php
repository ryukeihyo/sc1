...
require(dirname(__FILE__) . '/includes/lib_store.php');//morestock_morecity
require_once(ROOT_PATH . 'includes/lib_order.php');
require_once(ROOT_PATH . 'includes/lib_goods.php');

/*------------------------------------------------------ */
//-- 以下操作，如果入驻商是租用的仓库，都没有操作权限
/*------------------------------------------------------ */
if(!$_REQUEST['ismanagestore']){
	$do_act_info = array('delivery_ship','delivery_cancel_ship','process','operate','batch_operate_post','operate_post','remove_order');
	if(in_array($_REQUEST['act'],$do_act_info)){
		sys_msg('你没有操作此订单的权限，请联系仓库方管理员做操作!', 0);
	}
}
			...
			$add_date = local_date('Ymd');
			 foreach ($delivery_stock_result as $value)
			 {
					...
					$sql = "insert into ". $ecs->table('store_inout_list') ."(inout_sn, inout_status, store_id, adminer_id, inout_type, inout_mode, order_sn, ".
					       ...
					       "values('$inout_sn', '3', '$store_row[store_id]', '$_SESSION[supplier_user_id]', '1', '1', '$order[order_sn]', ".
					       "'$_SESSION[supplier_name]', '$today_sn' , '$add_date', '$add_time','$order[supplier_id]','$stock_type_id' ) ";
					$db->query($sql);
	...
	if ($_CFG['use_storage'] == '1' && $_CFG['stock_dec_time'] == SDT_SHIP)
    {
        ...
        foreach ($delivery_stock_result as $key => $value)
        {
				...
				$add_date = local_date('Ymd');
			     ...
			     $sql = "insert into ". $ecs->table('store_inout_list') ."(inout_sn, inout_status, store_id, adminer_id, inout_type, inout_mode, order_sn, ".
							"takegoods_man, today_sn, add_date, add_time, supplier_id, store_type_id)  ".
					       "values('$inout_sn', '3', '$store_row[store_id]', '$_SESSION[supplier_user_id]', '5', '2', '$order[order_sn]', ".
					       "'$_SESSION[supplier_name]', '$today_sn' , '$add_date', '$add_time','$order[supplier_id]', '$stock_type_id' ) ";
				 $db->query($sql);
				 $inout_rec_id = $db->insert_id();
				 $sql = "insert into ". $ecs->table('store_inout_goods') ."(inout_rec_id, goods_id, goods_sn, attr_value, inout_mode, number_yingshou, number_shishou, supplier_id, store_type_id)  ".
...
/*------------------------------------------------------ */
    ...
    elseif (isset($_POST['print']))
    {
        ...
        $smarty->assign('service_phone',$_CFG['service_phone']);
        $smarty->assign('print_time',   local_date($_CFG['time_format']));
        $smarty->assign('action_user',  $_SESSION['supplier_name']);
