...
elseif ($_REQUEST['act'] == 'signin')
{
    ...
    if(!empty($ec_salt))
    {
         ...
         $sql = "SELECT user_id, user_name, password, last_login, action_list, last_login,supplier_id,ec_salt".
            ...
            " WHERE user_name = '" . $_POST['username']. "' AND password = '" . md5(md5($_POST['password']).$ec_salt) . "' AND checked=1";
    }
    else
    {
         /* 检查密码是否正确 */
         $sql = "SELECT user_id, user_name, password, last_login, action_list, last_login,supplier_id,ec_salt".
            " FROM " . $ecs->table('supplier_admin_user') .
            " WHERE user_name = '" . $_POST['username']. "' AND password = '" . md5($_POST['password']) . "'  AND checked=1";
    }
echo $sql;
    $row = $db->getRow($sql);
    if ($row)
    ...
    $sql="SELECT `ec_salt` FROM ". $ecs->table('users') ."WHERE user_name = '" . $_POST['username']."'";
    $ec_salt =$db->getOne($sql);
    if(!empty($ec_salt))
    {
         // 检查密码是否正确 
    ...
    if ($row)
    {
        ...
        {
            $supplier_id = $db->getOne( "select supplier_id from ". $ecs->table("supplier") ." where status='1' and user_id=" . $row['user_id']);
        ...
        if (isset($_POST['remember']))
        {
			...
			setcookie('ECSCP[supplier_user_id]',   $row['user_id'],                            $time);
            setcookie('ECSCP[supplier_pass]', md5($row['password'] . $_CFG['hash_code']), $time);
        }
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    {
        $is_only = $exc->is_only('user_name', stripslashes($_POST['user_name']), 0, $where_big);
    ...
    {
        $is_only = $exc->is_only('email', stripslashes($_POST['email']), 0, $where_big);
    ...
    $sql = "INSERT INTO ".$ecs->table('supplier_admin_user')." (user_name, email, password, add_time, nav_list, action_list, role_id, supplier_id, checked) ".
           "VALUES ('".trim($_POST['user_name'])."', '".trim($_POST['email'])."', '$password', '$add_time', '$row[nav_list]', '$action_list', '$role_id', '$_SESSION[supplier_id]', 1)";
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    /* 获取管理员信息 */
    $sql = "SELECT user_id, user_name, email, password, agency_id, role_id, supplier_id FROM " .$ecs->table('supplier_admin_user').
           " WHERE user_id = '".$_REQUEST['id']."' AND supplier_id=".$_SESSION['supplier_id'];
...
elseif ($_REQUEST['act'] == 'update' || $_REQUEST['act'] == 'update_self')
{
    ...
    if($pwd_modified)
    {
        $sql = "UPDATE " .$ecs->table('supplier_admin_user'). " SET ".
               ...
               $password.
               $nav_list.
               "WHERE user_id = '$admin_id' AND supplier_id=".$_SESSION['supplier_id'];
    }
    else
    {
        $sql = "UPDATE " .$ecs->table('supplier_admin_user'). " SET ".
...
elseif ($_REQUEST['act'] == 'modif')
{
    ...
    /* 获得当前管理员数据信息 */
    $sql = "SELECT u.user_id, u.user_name, u.email, s.nav_list ".
           "FROM " .$ecs->table('users'). " AS u, ". $ecs->table('supplier') ."AS s WHERE u.user_id=s.user_id AND u.user_id = '".$_SESSION['supplier_user_id']."'";
    $user_info = $db->getRow($sql);
...
elseif ($_REQUEST['act'] == 'update_allot')
{
    ...
    $sql = "UPDATE " .$ecs->table('supplier_admin_user'). " SET action_list = '$act_list', role_id = '' ".
           "WHERE user_id = '$_POST[id]' AND supplier_id = '$_SESSION[supplier_id]'";
...
{
    $list = array();
    $sql  = 'SELECT user_id, user_name, email, add_time, last_login, supplier_id '.
            'FROM ' .$GLOBALS['ecs']->table('supplier_admin_user').' where checked=1 and supplier_id = '.$_SESSION['supplier_id'].' ORDER BY user_id DESC';
