...
if ($_REQUEST['act'] == 'list')
{
	...
	if(!$_REQUEST['ismanagestore']){
		...
		$sql="SELECT a.rec_id, a.admin_id FROM ".$ecs->table('store_adminer')." AS a ".
					"left join ". $ecs->table('store_main') ." as m on a.store_id=m.store_id  WHERE a.admin_id='$_SESSION[supplier_user_id]' and a.supplier_id='$_REQUEST[storetypeid]' and m.parent_id>0";
	...
	/* 入库类型 */
	$sql = "select type_id, type_name from ". $ecs->table('store_inout_type') ." where in_out=2 and supplier_id in (-1,".$_REQUEST['storetypeid'].") order by type_id asc";
...
if ($_REQUEST['act'] == 'insert')
{
    ...
    $sql = "INSERT INTO ".$ecs->table('store_inout_list')."(inout_sn, store_id, adminer_id,  inout_type, inout_mode, order_sn,  takegoods_man, ".
            ...
            "VALUES ('$inout_sn', '$store_id', '$_SESSION[supplier_user_id]', '$_POST[inout_type]', '2', '$_POST[order_sn]', '$_POST[takegoods_man]', ".
	...
	foreach ($_POST['goods_id']  AS $gkey=>$gval)
	{
			...
			$sql = "insert into ". $ecs->table('store_inout_goods') ." (inout_rec_id, goods_id, goods_sn, inout_mode, attr_value, number_yingshou, number_shishou, supplier_id, store_type_id) ".
						"values('$inout_rec_id',  '$goods_id', '$goods_sn', '2', '$attr_value', '$number_yingshou', '$number_shishou', '$_SESSION[supplier_id]', '$_REQUEST[storetypeid]')";
			$db->query($sql);
	}    
...
if ($_REQUEST['act'] == 'check' or $_REQUEST['act'] == 'view')
{
	...
	if ($inout)
	{
		...
		{
			$sql="select a.rec_id from ". $ecs->table('store_main') ." AS m left join ". $ecs->table('store_adminer') ." AS a on m.parent_id=a.store_id where m.supplier_id=".$_REQUEST['storetypeid']." and m.store_id='$inout[store_id]' and a.admin_id='$_SESSION[supplier_user_id]' ";
            $rec_id = $db->getOne($sql);
			if (!$rec_id)
			{
				sys_msg('对不起，您没有审核权限！');
		...
		$sql="select admin_name from ". $ecs->table('store_adminer') ." where supplier_id=".$_REQUEST[storetypeid]." and store_id='$inout[store_id]' ";
...
if ($_REQUEST['act'] == 'edit')
{
	...
	/* 出入库类型 */
	$sql = "select type_id, type_name from ". $ecs->table('store_inout_type') ." where in_out=2 and supplier_id=".$_REQUEST['storetypeid']." order by type_id asc";
	$inout_type_list = $db->getAll($sql);
	$smarty->assign('inout_type_list', $inout_type_list);
...
if ($_REQUEST['act'] =='update')
{
	...
	foreach ($_POST['goods_id']  AS $gkey=>$gval)
	{
			...
			$number_shishou = $_POST['number_shishou'][$gkey];
			$sql = "insert into ". $ecs->table('store_inout_goods') ." (inout_rec_id, goods_id, goods_sn, inout_mode, attr_value, number_yingshou, number_shishou, supplier_id, store_type_id ) ".
						"values('$rec_id',  '$goods_id', '$goods_sn', '2', '$attr_value', '$number_yingshou', '$number_shishou', '$_SESSION[supplier_id]', '$_REQUEST[storetypeid]')";
			$db->query($sql);
	} 
