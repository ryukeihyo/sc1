...
if ($_REQUEST['act'] == 'get_store_admin')
{
	...
	$opt['admin_isme'] = '1';
	if($_REQUEST['ismanagestore']){
		$sql = "select rec_id from ". $ecs->table('store_adminer') ." where admin_id='$_SESSION[supplier_user_id]' and store_id = '$store_id' ";
...
if ($_REQUEST['act'] == 'get_goodslist_byOrdersn')
{
	...
	{
		$sql = "select  b.goods_id,  b.goods_sn, b.goods_attr, b.goods_attr_id, b.goods_number,  g.goods_name, g.goods_thumb ".
				" from ". $ecs->table('order_goods') ." AS b left  join ". $ecs->table('goods') ." AS g on b.goods_id=g.goods_id ".
				" where b.order_id='$back_id' ";
		...
		while ($row_ecshop120 = $db->fetchRow($res_ecshop120))
		{
			...
			$row_ecshop120['goods_attr'] = str_replace(',', '|', $row_ecshop120['goods_attr_id']);
...
if ($_REQUEST['act'] == 'get_goodsInfo_bysn')
{
    ...
    $sql = "select goods_id, goods_name, goods_thumb from ". $ecs->table('goods') ." where goods_sn= '$goods_sn' ";
...
if ($_REQUEST['act'] == 'list')
{
    /* 取得过滤条件 */
    $filter = array();
    $smarty->assign('ur_here',      $_LANG['03_store_inout_out']);
    
    if(!$_REQUEST['ismanagestore']){
		$smarty->assign('action_link',  array('text' => $_LANG['store_inout_add_in'], 'href' => 'store_inout_out.php?act=add'));
	}else{
		$sql="SELECT a.rec_id, a.admin_id FROM ".$ecs->table('store_adminer')." AS a ".
					"left join ". $ecs->table('store_main') ." as m on a.store_id=m.store_id  WHERE a.supplier_id= ".$_REQUEST[storetypeid]." and a.admin_id='$_SESSION[supplier_user_id]' and m.parent_id>0";
	...
	/* 出库类型 */
	$sql = "select type_id, type_name from ". $ecs->table('store_inout_type') ." where in_out=1 and supplier_id in (-1,".$_REQUEST[storetypeid].") order by type_id asc";
	$inout_type_list = $db->getAll($sql);
	$smarty->assign('inout_type_list', $inout_type_list);
    
	/* 仓库 */
	$sql="select store_id, store_name from ". $ecs->table('store_main')." where supplier_id=".$_REQUEST[storetypeid]." and parent_id='0' ";
	$store_list=$db->getAll($sql);
	$smarty->assign('store_list', $store_list);
	/* 库房 */
	$sid=$_REQUEST['sid'] ? intval($_REQUEST['sid']) : 0;
	$sql = " select store_id,store_name  from ". $ecs->table('store_main')." where supplier_id=".$_REQUEST[storetypeid]." and ";
	...
	$smarty->assign('script_file', 'store_inout_out.php');

    assign_query_info();
    $smarty->display('store_inout_out_list.htm');
}
...
{
    check_authz_json('inout_out_manage');
...
if ($_REQUEST['act'] == 'add')
{
	...
	if ($inout['store_id'])
	{
		$inout['sub_id_list'] = $db->getAll("select store_id, store_name from ". $ecs->table('store_main') ." where supplier_id=".$_REQUEST['storetypeid']." and parent_id='$inout[store_id]' ");
	}
	...
	$inout_sn =  'ck'.$inout['add_date'] . $inout_sn;
	...
	if(!$_REQUEST['ismanagestore']){
	    /* 取得店铺名称 */
	    $sql = "select value from ". $ecs->table('supplier_shop_config') ." where supplier_id=".$_SESSION['supplier_id']." and code='shop_name'";
	    $supp = $db->getRow($sql);
	    if($supp['value']){
	    	$inout['takegoods_man'] = $supp['value'];
	    }
    }

    $smarty->assign('inout',     $inout);
    $smarty->assign('ur_here',     $_LANG['store_inout_add_in']);
    $smarty->assign('action_link', array('text' => $_LANG['store_inout_list_in'], 'href' => 'store_inout_out.php?act=list&' . list_link_postfix()));
    $smarty->assign('form_action', 'insert');

    assign_query_info();
    $smarty->display('store_inout_out_info.htm');
}
...
{
	$smarty->assign('ur_here',     '批量导入出库商品');
    $smarty->assign('action_link', array('text' => $_LANG['store_inout_list_in'], 'href' => 'store_inout_out.php?act=list&' . list_link_postfix()));
...
if ($_REQUEST['act'] == 'insert')
{
	...
	{
		$attr_sql = "";
		if(isset($_POST['goods_attr'][$gkey]) && !empty($_POST['goods_attr'][$gkey])){
			$attr_sql = " and goods_attr='".$_POST['goods_attr'][$gkey]."'";
		}
		$sql = "select store_number from ".$ecs->table('store_goods_stock')." where store_id=".$_POST['sub_id']." and goods_id=".$gval.$attr_sql;
		$info = $db->getOne($sql);
		if($info){
			if($_POST['number_shishou'][$gkey] > $info['store_number']){
				$warning.='['.$_POST['goods_sn'][$gkey].']库存不足<br>';
			}
		}else{
			$warning.='['.$_POST['goods_sn'][$gkey].']库存不存在<br>';
		}
	}
	if(!empty($warning)){
		sys_msg($warning);
    ...
    $sql = "INSERT INTO ".$ecs->table('store_inout_list')."(inout_sn, store_id, adminer_id,  inout_type, inout_mode, order_sn,  takegoods_man, ".
                " inout_status, today_sn, add_date, add_time, supplier_id, store_type_id) ".
            "VALUES ('$inout_sn', '$_POST[sub_id]', '$_SESSION[supplier_user_id]', '$_POST[inout_type]', '1', '$_POST[order_sn]', '$_POST[takegoods_man]', ".
                " '1', '$today_sn', '$add_date', '$add_time', '$_SESSION[supplier_id]', '$_REQUEST[storetypeid]')";
    $db->query($sql);
	...
	foreach ($_POST['goods_id']  AS $gkey=>$gval)
	{
			...
			$sql = "insert into ". $ecs->table('store_inout_goods') ." (inout_rec_id, goods_id, goods_sn, inout_mode, attr_value,  number_yingshou, number_shishou, supplier_id, store_type_id ) ".
						"values('$inout_rec_id', '$goods_id', '$goods_sn', '1', '$attr_value', '$number_yingshou', '$number_shishou', '$_SESSION[supplier_id]', '$_REQUEST[storetypeid]')";
			$db->query($sql);
	}    

    $link[0]['text'] = $_LANG['continue_add'];
    $link[0]['href'] = 'store_inout_out.php?act=add';

    $link[1]['text'] = $_LANG['back_list_in'];
    $link[1]['href'] = 'store_inout_out.php?act=list';
...
if ($_REQUEST['act'] == 'check' or $_REQUEST['act'] == 'view')
{
	...
	if ($inout)
	{
		...
		{
			$sql="select a.rec_id from ". $ecs->table('store_main') ." AS m left join ". $ecs->table('store_adminer') ." AS a on m.parent_id=a.store_id where m.store_id='$inout[store_id]' and a.admin_id='$_SESSION[supplier_user_id]' ";
			$rec_id = $db->getOne($sql);
			if (!$rec_id)
			{
				sys_msg('对不起，您没有审核权限！');
    ...
    assign_query_info();
    $smarty->display('store_inout_out_view.htm');
}
...
if ($_REQUEST['act'] == 'check_insert')
{
		...
		/* 获取入库单信息 */
		$sql = "select rec_id, store_id  from ". $ecs->table('store_inout_list') ." where rec_id='$rec_id' ";
		...
		{
			$sql_sh="select a.rec_id from ". $ecs->table('store_main') ." AS m left join ". $ecs->table('store_adminer') ." AS a on m.parent_id=a.store_id where m.store_id='$inout_row[store_id]' and a.admin_id='$_SESSION[supplier_user_id]' ";
		...
		{
			$upre = update_stock_out($rec_id, $inout_row['store_id']);  //更新库存表
			if($upre['error'])
			{
		...
		$sql="insert into  ". $ecs->table('store_inout_note').
				" (inout_rec_id, adminer_id, action_val, inout_status, inout_note, add_time, supplier_id, store_type_id)".
				" values('$rec_id', '$_SESSION[supplier_user_id]', '$action_val', '$inout_status', '$inout_note', '$add_time', '$_SESSION[supplier_id]', '$_REQUEST[storetypeid]')";
		$db->query($sql);


		$link[0]['text'] = $_LANG['back_list_in'];
		$link[0]['href'] = 'store_inout_out.php?act=list';
...
if ($_REQUEST['act'] == 'edit')
{
    ...
    $inout = $db->GetRow($sql);	
	if($inout['adminer_id']!=$_SESSION['supplier_user_id'])
	{
		sys_msg('您无权编辑这个出库单！');
	}
	$inout['add_time_date'] = local_date($GLOBALS['_CFG']['time_format'], $inout['add_time']);
...
if ($_REQUEST['act'] =='update')
{
	...
	foreach ($_POST['goods_id']  AS $gkey=>$gval)
	{
			...
			$number_shishou = $_POST['number_shishou'][$gkey];
			$sql = "insert into ". $ecs->table('store_inout_goods') ." (inout_rec_id, goods_id, goods_sn, inout_mode, attr_value, number_yingshou, number_shishou, supplier_id, store_type_id ) ".
						"values('$rec_id',  '$goods_id', '$goods_sn', '1', '$attr_value', '$number_yingshou', '$number_shishou', '$_SESSION[supplier_id]', '$_REQUEST[storetypeid]')";
			$db->query($sql);
	} 

	$link[0]['text'] = $_LANG['back_list_in'];
	$link[0]['href'] = 'store_inout_out.php?act=list&' . list_link_postfix();
...
elseif ($_REQUEST['act'] == 'remove')
{
	...
	{
		if ($row_inout['adminer_id']!=$_SESSION['supplier_user_id'])
		{
			sys_msg('对不起，您没有删除权限');
		}
...
function get_inoutlist()
{
    ...
    if ($result === false)
    {
		...
		$storeid_list="";
		$where1 = ($_REQUEST['ismanagestore']) ? " and admin_id = '$_SESSION[supplier_user_id]' " : "";
		$sql1 = "select store_id from " . $GLOBALS['ecs']->table('store_adminer') ." where supplier_id=".$_REQUEST['storetypeid'].$where1;
        ...
        $sql = 'SELECT *  '.
               ...
               'WHERE inout_mode=1 ' .$where. ' ORDER by '.$filter['sort_by'].' '.$filter['sort_order'];
    ...
    while ($rows = $GLOBALS['db']->fetchRow($res))
    {
    	...
    	$rows['tjsq'] = 0;
		if($rows['inout_status'] == 1 && $_REQUEST['ismanagestore']){//是否有提交申请的权利
			$sql_sh="select rec_id from ". $GLOBALS['ecs']->table('store_adminer') ." where store_id='$rows[store_id]' and admin_id='$_SESSION[supplier_user_id]' ";
