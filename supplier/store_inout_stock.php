...
if ($_REQUEST['act'] == 'insert')
{
    ...
    $sql = "INSERT INTO ".$ecs->table('store_inout_list')."(inout_sn, store_id, inout_type, inout_mode, order_sn,  takegoods_man, ".
                " today_sn, add_date, add_time, supplier_id, store_type_id) ".
            "VALUES ('$inout_sn', '$_POST[sub_id]', '$_POST[inout_type]', '2', '$_POST[order_sn]', '$_POST[takegoods_man]', ".
                " '$today_sn', '$add_date', '$add_time', '$_SESSION[supplier_id]', '$_REQUEST[storetypeid]')";
    $db->query($sql);
...
function get_stocklist()
{
    ...
    if ($result === false)
    {
        ...
        {  
			$goods_id="0";
			$sql1="select goods_id from ". $GLOBALS['ecs']->table('goods') ." where supplier_id=".$_SESSION['supplier_id']." and goods_sn='$filter[goods_sn]' ";
			$goods_id1= $GLOBALS['db']->getOne($sql1);
			$goods_id .=",".$goods_id1;
			$where .= " AND goods_id in ($goods_id)";
        }
		elseif (!empty($filter['goods_name']))
        {
			$sql1="select goods_id from ". $GLOBALS['ecs']->table('goods') ." where supplier_id=".$_SESSION['supplier_id']." and goods_name LIKE '%" . mysql_like_quote($filter['goods_name']) . "%' ";
