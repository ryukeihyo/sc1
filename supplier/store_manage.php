...
if ($_REQUEST['act'] == 'shipping_area' )
{
	...
	/* 运费模板列表 */
	$sql="select * from ".$ecs->table('store_shipping_demo')." ORDER BY demo_id DESC";  
	$res = $db->query($sql);
	$demo_list = array();
...
elseif($_REQUEST['act'] == 'add_shipping_area_new'){
	...
	foreach($citys as $key => $val){
		...
		if(count($arr)>3){
			...
			$linshi['supplier_id'] = $_SESSION['supplier_id'];
			$linshi['store_type_id'] = $_REQUEST['storetypeid'];
			$sql="select count(*) from ". $ecs->table('store_shipping_region') ." where supplier_id='$_SESSION[supplier_id]' and store_id='$parent_id' ".$where;
			if ($db->getOne($sql))
			{
				unset($linshi);
			}else{
...
elseif ($_REQUEST['act'] == 'add_shipping_area')
{
    ...
    /* 查看是否重复 */
	$sql="select count(*) from ". $ecs->table('store_shipping_region') ." where supplier_id='$_SESSION[supplier_id]' and store_id='$parent_id' and province='$province' and city='$city' and district='$district' and xiangcun='$xiangcun' ";
    if ($db->getOne($sql))
    {
        make_json_error($_LANG['shipping_area_exist']);
    }	

    $sql = "INSERT INTO " . $ecs->table('store_shipping_region') . " (store_id, province, city, district, xiangcun, supplier_id, store_type_id) ".
           "VALUES ('$parent_id', '$province', '$city', '$district', '$xiangcun', '$_SESSION[supplier_id]', '$_REQUEST[storetypeid]')";
...
if ($_REQUEST['act'] == 'shipping_area_fee' )
{
	...
	$sql="select r.store_id, r.province, r.city, r.district, s.store_name from ". $ecs->table('store_shipping_region') ." AS r".
	...
	$sql="select f.fee_id, f.shipping_id, s.shipping_name from ". $ecs->table('store_shipping_fee') ." AS f ".
	...
	/* 运费模板列表 */
	$sql="select * from ".$ecs->table('store_shipping_demo')." where supplier_id=".$_SESSION[supplier_id]." ORDER BY demo_id DESC";  
	$res = $db->query($sql);
	$demo_list = array();
    while ($rows = $db->fetchRow($res))
    {
...
if ($_REQUEST['act'] == 'store_set_adminer' )
{
	...
	$sql="select * from ".$ecs->table('store_adminer')." where supplier_id=".$_SESSION[supplier_id]." and store_id='$store_id' ";
...
if ($_REQUEST['act'] == 'store_set_adminer_save' )
{
	...
	if (is_array($admin_list))
	{
		foreach ($admin_list AS $admin_item)
		{
			...
			$sql = "INSERT INTO " . $ecs->table('store_adminer') . " (store_id, admin_id, admin_name, mobile, tel, supplier_id, store_type_id) ".
           "VALUES ('$store_id', '$admin_item', '$admin_name', '$mobile', '$tel', '$_SESSION[supplier_id]', '$_REQUEST[storetypeid]')";
			$db->query($sql);
		}
	}
...
if ($_REQUEST['act'] == 'list_sub')
{
	...
	$parent_name= $db->getOne("select store_name from ". $ecs->table('store_main') ." where supplier_id=".$_REQUEST[storetypeid]." and store_id='$parent_id' ");
	$smarty->assign('ur_here',     $_LANG['list_sub']." - [$parent_name]");
	$smarty->assign('full_page',    1);

	$sql="select * from ".$ecs->table('store_main')." where supplier_id=".$_REQUEST[storetypeid]." and parent_id='$parent_id' order by store_id desc ";
...
if ($_REQUEST['act'] == 'save_sub' )
{
	...
	$sql = "INSERT INTO " . $ecs->table('store_main') . " (parent_id, store_name, province, city, district, mianji, supplier_id, store_type_id) ".
           "VALUES ('$parent_id', '$store_name', '$_REQUEST[province]', '$_REQUEST[city]', '$_REQUEST[district]',  '$_REQUEST[mianji]', '$_SESSION[supplier_id]', '$_REQUEST[storetypeid]')";
	$db->query($sql);
...
elseif ($_REQUEST['act'] == 'add_store')
{
    ...
    /* 查看库房是否重复 */
    if (!$exc->is_only('store_name', $store_name, 0, "supplier_id = '$_SESSION[supplier_id]' and parent_id = '$parent_id'"))
    {
        make_json_error($_LANG['store_name_exist']);
    }

    $sql = "INSERT INTO " . $ecs->table('store_main') . " (parent_id, store_name, supplier_id, store_type_id) ".
           "VALUES ('$parent_id', '$store_name', '$_SESSION[supplier_id]', '$_REQUEST[storetypeid]')";
    if ($GLOBALS['db']->query($sql, 'SILENT'))
    {
        /* 获取仓库列表 */
        $store_arr = store_list($keyword);
...
{
    $area_arr = array();
	$where = " where supplier_id=".$_REQUEST['storetypeid']." and parent_id=0 ";
    if ($keyword)
	{
