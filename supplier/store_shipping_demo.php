...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    $sql = "INSERT INTO ".$ecs->table('store_shipping_demo')."(shipping_id, shipping_name, configure, supplier_id, store_type_id) ".
           "VALUES ('$shipping_id', '$shipping_name', '$configure', '$_SESSION[supplier_id]', '$_REQUEST[storetypeid]')";
    $db->query($sql);
...
function get_demolist()
{
    ...
    if ($result === false)
    {
        ...
        /* 记录总数以及页数 */        
        $sql = "SELECT COUNT(*) FROM ".$GLOBALS['ecs']->table('store_shipping_demo')." WHERE supplier_id=".$_REQUEST['storetypeid'];

        $filter['record_count'] = $GLOBALS['db']->getOne($sql);
        $filter = page_and_size($filter);

        /* 查询记录 */
       $sql = "SELECT * FROM ".$GLOBALS['ecs']->table('store_shipping_demo')." WHERE supplier_id=".$_REQUEST['storetypeid']." ORDER BY demo_id DESC";       
