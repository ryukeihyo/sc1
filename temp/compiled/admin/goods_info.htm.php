...
<?php echo $this->smarty_insert_scripts(array('files'=>'../js/utils.js,selectzone_bd.js,colorselector.js')); ?>
...
?>
		 ...
		 <!--代码增加_start byjdy-->
		  <tr>
            <td class="label" ><font color=#ff3300>是否显示在频道首页：</font></td>
            <td ><input type="checkbox" name="is_catindex" value="1" <?php if ($this->_var['goods']['is_catindex']): ?>checked="checked"<?php endif; ?> /> 打勾表示在频道首页显示，否则不显示。</td>
          </tr>
		  <!--代码增加_end byjdy-->
        ...
        <!-- <?php if ($this->_var['goods']['supplier_id']): ?> --> 
		  <tr>
            ...
            <td><input type="radio" name="supplier_status" value="1" <?php if ($this->_var['goods']['supplier_status'] == '1'): ?>checked=checked<?php endif; ?>>审核通过<input type="radio" name="supplier_status" value="0" <?php if ($this->_var['goods']['supplier_status'] == '0'): ?>checked=checked<?php endif; ?> >未审核<input type="radio" name="supplier_status" value="-1" <?php if ($this->_var['goods']['supplier_status'] == '-1'): ?>checked=checked<?php endif; ?> >审核未通过</td>
			</tr>
		<tr>
            <td class="label">审核消息</td>
            <td><textarea name="supplier_status_txt" rows=4 cols=50><?php echo $this->_var['goods']['supplier_status_txt']; ?></textarea></td>
			</tr>
           <!--<?php endif; ?>-->
...
<!--代码修改_start By www.ecshop68.com  将 商品相册 这部分代码完全修改成下面这样-->
        <table width="100%" id="gallery-table" style="display:none" align="center">
          ...
          <tr>
            <td>			
			<style>
			.attr-color-div{width:100%;background: #BBDDE5; text-indent: 10px;  height: 26px;  padding-top: 1px;}
			.attr-front {
			background: #CCFF99;
			line-height: 24px;
			font-weight: bold;
...
?>
			...
			<?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
              ...
              <iframe  name="attr_upload" src="attr_img_upload.php?goods_id=<?php echo $this->_var['goods']['goods_id']; ?>&goods_attr_id=<?php echo $this->_var['color_list_www_ecshop68_com']['0']['goods_attr_id']; ?>" frameborder=1 scrolling=no width="100%" height="auto" style="min-height:630px; border:1px #eee solid; margin-top:5px;">
			  </iframe>
