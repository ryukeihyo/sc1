...
<div class="tab-div">
    ...
    <div id="tabbar-div">
      <p>
        ...
        <span class="tab-back" id="linkgoods-tab">售后保障</span>
        <span class="tab-back" id="mix-tab"><?php echo $this->_var['lang']['tab_mix']; ?></span><?php if ($this->_var['goods_type_list']): ?><span
        class="tab-back" id="properties-tab"><?php echo $this->_var['lang']['tab_properties']; ?></span><?php endif; ?><span
        class="tab-back" id="gallery-tab"><?php echo $this->_var['lang']['tab_gallery']; ?></span>
      </p>
    </div>
    ...
    <div id="tabbody-div">
      <form enctype="multipart/form-data" action="" method="post" name="theForm" >
		...
		<script>
		function catsel_ecshop120(obj, level)
		{
			...
			Ajax.call('goods.php?act=get_catsel_ecshop120&sjs='+Math.random(), "parentid=" + parentid + "&level=" + level, catsel_ecshop120_response, "GET", "JSON");
			//alert(parentid);
		}
		function catsel_ecshop120_response(res)
		{
		...
		<tr>
            <td class="label"><?php echo $this->_var['lang']['lab_goods_cat']; ?></td>
			<td >
			<?php if ($this->_var['is_add_zhyh']): ?>
				<div id="catsel1" >
				<input type="hidden" name="cat_level_id" id="cat_level_id" value="1">
				<select name="cat_id_1" style="float:left;display:inline;margin-left:3px;" size=8 onchange="catsel_ecshop120(this, 1)">
				<option value="0" style="background:#dce6f0;">请选择&nbsp;&nbsp;&nbsp;&nbsp;</option>
				<?php $_from = $this->_var['pcats']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'pcat');if (count($_from)):
    foreach ($_from AS $this->_var['pcat']):
?>
				<option value="<?php echo $this->_var['pcat']['id']; ?>"><?php echo $this->_var['pcat']['name']; ?>&nbsp;&nbsp;</option>
				<?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
				</select>
					<div id="catsel2" style="float:left;display:inline;margin-left:3px;">				
					</div>
				</div>

			<?php else: ?>
			<?php $_from = $this->_var['catoptions']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('ckey', 'coptions');if (count($_from)):
    foreach ($_from AS $this->_var['ckey'] => $this->_var['coptions']):
?>
			<?php if ($this->_var['coptions']): ?>
				<div id="catsel<?php echo $this->_var['ckey']; ?>" >
					<?php if ($this->_var['ckey'] == 1): ?>
					<input type="hidden" name="cat_level_id" id="cat_level_id" value="<?php echo $this->_var['levels']; ?>">
					<?php endif; ?>
					<select name="cat_id_<?php echo $this->_var['ckey']; ?>" style="float:left;display:inline;margin-left:3px;" size=8 onchange="catsel_ecshop120(this, <?php echo $this->_var['ckey']; ?>)">
					<option value="0" style="background:#dce6f0;">请选择&nbsp;&nbsp;&nbsp;&nbsp;</option>
					<?php $_from = $this->_var['coptions']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'ccat');if (count($_from)):
    foreach ($_from AS $this->_var['ccat']):
?>
					<option value="<?php echo $this->_var['ccat']['cat_id']; ?>" <?php echo $this->_var['ccat']['selected']; ?>><?php echo $this->_var['ccat']['cat_name']; ?>&nbsp;&nbsp;</option>
					<?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
					</select>
				</div>
			<?php endif; ?>
		  ...
		  <td >
		  <div  style="float:left;border:1px solid #000;width:auto;height:140px;padding:5px 5px 5px 0; " class="divScroll">
...
?>
        ...
        <table width="90%" id="linkgoods-table" style="display:none" align="center">
            <tr>
                <td><?php echo $this->_var['FCKeditor_after']; ?></td>
            </tr>
        </table>
...
function getElementsByClassName(n)
{
  ...
  function validate(goods_id)
  {
	  ...
	  var cat_level_id = document.forms['theForm'].elements['cat_level_id'].value;

      validator.required('goods_name', goods_name_not_null);
      if (document.forms['theForm'].elements['cat_id_'+ cat_level_id].value == 0)
      {
