...
<table width="100%" cellpadding="3" cellspacing="1">
  ...
  <tr>
	...
	<?php if ($this->_var['cfg_stock_dec_time'] == '2'): ?>
...
?>
	...
	<?php if ($this->_var['cfg_stock_dec_time'] == '2'): ?>
	<td >
	<?php if ($this->_var['goods']['send_store_number']): ?>
	<input type="text" name="yd_<?php echo $this->_var['goods']['rec_id']; ?>" id="yd_<?php echo $this->_var['goods']['rec_id']; ?>" size=5 value="<?php echo $this->_var['goods']['send_store_number']; ?>">
	<input type="hidden" name="yds_<?php echo $this->_var['goods']['rec_id']; ?>" id="yds_<?php echo $this->_var['goods']['rec_id']; ?>" size=5 value="<?php echo $this->_var['goods']['send_store_id']; ?>">
	<input type="button" value="取消预定" onclick="kucun_quxiaoyuding(<?php echo $this->_var['goods']['rec_id']; ?>);">
	<?php else: ?>
	<input type="text" name="yd_<?php echo $this->_var['goods']['rec_id']; ?>" id="yd_<?php echo $this->_var['goods']['rec_id']; ?>" size=5 value="<?php echo $this->_var['goods']['goods_number']; ?>">
	<input type="button" value="库存预定" onclick="kucun_yuding(<?php echo $this->_var['goods']['rec_id']; ?>);">
	<?php endif; ?>
	</td>
	<?php endif; ?>
...
<table cellpadding="3" cellspacing="1">
  ...
  <tr>
    ...
    <td colspan="2">
    <input name="invoice_no" type="text" style="color:#00F" value="请输入快递单号" /><input name="to_shipping" type="submit" style="color:#F00" value="一键发货" class="button" />
    </td>
    <td><div align="right"><strong style="color:#c00">若一键发货请忽略此操作</strong></div></td><td colspan="2">
    <?php else: ?>
    <td colspan="5">
    <?php endif; ?>
    <!-- 一键发货end 修改 by www.68ecshop.com -->  <?php if ($this->_var['operable_list']['confirm']): ?>
...
<script language="JavaScript">
 ...
 function kucun_yuding(rec_id)
 {
	  var send_store_number  = document.getElementById('yd_'+rec_id).value;
      var url="order.php?act=inout_out&rec_id="+rec_id+"&send_store_number="+send_store_number;
	  location.href=url;
 }
  function kucun_quxiaoyuding(rec_id)
 {
	  var send_store_number  = document.getElementById('yd_'+rec_id).value;
	  var send_store_id  = document.getElementById('yds_'+rec_id).value;
      var url="order.php?act=inout_in&rec_id="+rec_id+"&send_store_number="+send_store_number+"&send_store_id="+send_store_id;
	  location.href=url;
 }
